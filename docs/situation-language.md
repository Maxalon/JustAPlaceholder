...
| `gainControl` | `player` (the new controller), `obj`, `to`: `"eot"` for until end of turn (layer 2, 613.1b). The permanent doesn't change zones, so it is summoning sick for its new controller. |
| `flip` | `player`, `to`: `"win"` or `"lose"` (a coin flip the situation settles, 705.2). Without one, a card that flips reports both branches and asks which way it went. |
