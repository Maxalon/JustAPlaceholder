...
| `ask` | `object`, `to` = `trigger`, `survive` or `die`; or `player`, `to` = `playerSurvive`, `playerDie` or `playerWin` | A question about that permanent, answered in the outcome once everything has resolved. |
