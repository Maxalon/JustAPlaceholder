...
| `ask` | `object`, `to` = `trigger`, `survive`, `die`, `tapped`, `pt` or `control` (with `player`); or `player`, `to` = `playerSurvive`, `playerDie` or `playerWin` | A question about that permanent, answered in the outcome once everything has resolved. |
