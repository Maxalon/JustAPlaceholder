...
class SituationParser(private val names: NameIndex) {
    ...
    private fun readSentence(m: Marked, ctx: Ctx): Boolean {
        ...
        Regex("""\b(opponent|they|they're|opp|my opponent)(?: who)? (is at|are at|has|have|at|is on|are on) (\d+)(?: life)?\b""").find(t2)?.let { ctx.life[pronounPlayer(ctx)] = it.groupValues[3].toInt(); any = true; t2 = t2.replaceRange(it.range, it.groupValues[1]) }
    ...
    private fun isNoise(clause: String) = Regex("""^(what happens|what now|so|then|now|ok|okay|right|do i draw|does it work|is that right|correct|and|but|also|too|as well)\??$""").matches(clause.trim()) ||
    ...
    private fun readClause(clause0: String, m: Marked, ctx: Ctx): Boolean {
        ...
        Regex("""^(.+?)\s+(?:while|when|although|even though|given that) ((?:i|my|they|their|the opponent|my opponent|opponent|@\w+)\b.*)$""").find(clause0)?.let { r ->
            if (Regex("""\b(?:control|controls|have|has|got|is|are|out|in play)\b""").containsMatchIn(r.groupValues[2])) {
                val stateRead = readClause(r.groupValues[2], m, ctx)
                return readClause(r.groupValues[1], m, ctx) || stateRead
            }
        }
        // Leading connectives carry no information: "then my opponent…", "next, they…", "now I…", "so I…".
        var c = clause0.replace(Regex("""^(?:then|next|now|so|after that|afterwards|later|finally|also|meanwhile|after (?:blockers|blocks|attackers|attacks)(?: are declared)?|before (?:combat )?damage|in the (?:declare blockers|declare attackers|end|combat damage|beginning of combat) step|during (?:combat|the combat phase)|at (?:that|this) point|with (?:that|it|the trigger|the spell) on the stack|in response)\s*,?\s+"""), "")
        // "… in response to Lightning Bolt": that spell was cast first, by the other player, and is still on the stack.
        Regex("""\s+in response to (?:an? |the |their |my )?(c\d+)(?:'s)?$""").find(c)?.let { r ->
            ...
            if (card.display !in ctx.castCards) {
                ...
                if (guess != null) ctx.notes += "${card.display} was read as targeting ${ctx.objects.getValue(guess).card.name} (the thing the response protects); say otherwise if it targeted something else."
            }
        }
        ...
        Regex("""^(?:sacrifices?|sacs?|sacrificing|saccing) (?:an? |the |my |one |another )?(c\d+)(?: to (?:an? |the |my )?(c\d+)(?:'s ability)?)?(.*)$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
        ...
        Regex("""^taps? (?:an? |the |my )?(c\d+) for (?:mana|\{.*|[a-z]+ mana|[a-z]+)$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            val card = m.cards.getValue(r.groupValues[1]); val id = objectIdFor(card, ctx) ?: addObject(card, who, false, ctx)
        ...
        Regex("""^(?:draws?|drew|drawing) (a|an|\d+|two|three|four|five) cards?(?: (?:during|in|for|at) (?:their|my|the|his|her) draw step)?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
        ...
        Regex("""^(?:commander |my commander |the |an? |their )?(c\d+) (?:attacks?|swings?)(?: (?:at |into )?(.*))?$""").find(c)?.let { r ->
            val card = m.cards.getValue(r.groupValues[1])
    ...
    private fun emitCast(who: String, card: NameIndex.Entry, rest: String, m: Marked, ctx: Ctx) {
        ...
        val targets = if (overload) emptyList() else targetsIn(rest, m, ctx)
        ctx.events += EventSpec("cast", player = who, card = CardRef(name = card.display, oracleId = card.oracleId), targets = targets, modes = modes, to = if (overload) "overload" else null)
        ctx.lastActor = who
        ctx.lastVerb = "cast"
    ...
    private fun targetsIn(rest: String, m: Marked, ctx: Ctx): List<String> {
        ...
        Regex("""(?:the |my |their |an? )?(c\d+)""").find(seg)?.let { t ->
            ...
            val owner = when {
                ...
                else -> ctx.other(ctx.lastActor) ?: "me"
            }
