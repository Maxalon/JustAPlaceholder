...
class SituationParser(private val names: NameIndex) {
     ...
     */
    /** "in a four player game" / "my three opponents": that many opponents at the table, the extra ones named opponent 2, 3 … */
    private fun ensureOpponents(n: Int, ctx: Ctx) {
        if (n < 2) return
        ctx.usesOpp = true
        for (k in 2..n) ctx.players.putIfAbsent("opp$k", "opponent $k")
    }

    /** "I attack her with a 3/3": "her" after an attack is the planeswalker last named, when there is one (Elspeth, Liliana). */
    private fun planeswalkerFor(word: String, ctx: Ctx): String? {
        if (word !in setOf("him", "her", "it", "them")) return null
        val cand = listOfNotNull(ctx.lastMentioned) + ctx.objects.keys.reversed()
        return cand.firstOrNull { id -> ctx.objects[id]?.let { o -> (o.zone == null || o.zone == "battlefield") && o.card.name?.let { n -> names.lookup(Names.normalize(n))?.typeLine?.contains("Planeswalker", true) == true || n == "a planeswalker" } == true } == true }
    }

    /** Creatures already named in the sentence before a "board of N creatures" phrase: they are among the N. */
    private fun ctx0Creatures(t: String): Int = Regex("""\bc(\d+)\b""").findAll(t.substringBefore(" board of ")).map { it.groupValues[1] }.distinct().count { k -> currentMarked?.cards?.get("c$k")?.let { isCreatureName(it.display) } == true }
    private var currentMarked: Marked? = null
    ...
    private fun playerNames(sentences: List<String>): Map<String, String> {
        ...
        for (sentence in sentences) {
            ...
            for ((i, raw) in rawWords.withIndex()) {
                ...
                // "alice attacks bob with a 4/4": a common first name typed in lowercase is still a player.
                val lowerName = core.lowercase() in lowercaseNames && core == core.lowercase()
                if (!Regex("""[A-Z][a-z]{1,}""").matches(core) && !lowerName) continue
                ...
                if (actsLikePlayer) out[norm] = if (lowerName) core.replaceFirstChar { it.uppercase() } else core
            }
        }
        return out
    }

    private val playerNameStop = setOf("i", "my", "me", "we", "opponent", "opp", "they", "he", "she", "it", "the", "then", "now", "so", "if", "when", "what", "where", "why", "which", "who", "whom", "graveyards", "graveyard", "libraries", "library", "hands", "exile", "battlefield", "stack", "how", "will", "would", "could", "should", "am", "was", "were", "does", "do", "can", "is", "are", "at", "during", "on", "in", "after", "before", "also",
    ...
    /** Spells whose "take" is a discard: "I cast Thoughtseize and take their Counterspell". */
    private val discardSpells = setOf("thoughtseize", "duress", "inquisition of kozilek", "thought erasure", "cabal therapy", "unmask", "distress", "despise",
        "agonizing remorse", "appetite for brains", "hymn to tourach", "mind twist", "blackmail", "ostracize", "harsh scrutiny", "divest", "go blank", "wrench mind",
        "raven's crime", "funeral charm", "castigate", "thought hemorrhage", "lay bare the heart", "vendetta", "coercion", "persecute", "cabal therapist", "elderfang disciple", "burglar rat")
    private val lowercaseNames = setOf("alice", "bob", "carol", "dave", "eve", "mallory", "trent", "john", "jane", "mike", "sarah", "tom", "sam", "alex", "chris", "jake",
        "matt", "dan", "nick", "josh", "ben", "jim", "jeff", "tim", "kevin", "mark", "paul", "steve", "lisa", "emma", "max", "leo", "luke", "ryan", "adam", "andy", "tony",
        "jack", "joe", "jon", "bill", "ian", "liam", "noah", "olivia", "mia", "ava", "sophia", "anna", "kate", "amy", "greg", "brian", "eric", "james", "david", "peter", "fred", "george", "harry")
    ...
    private fun mark(sentence: String, short: Map<String, NameIndex.Entry> = emptyMap(), named: Map<String, String> = emptyMap()): Marked {
        ...
        val found = (full + normWords.indices.filter { it !in covered }.mapNotNull { i -> shortAt(i)?.let { NameIndex.Found(i, i + 1, it) } })
            ...
            .filter { f -> !(f.end - f.start == 1 && normWords[f.start] in cardTypeWords &&
                (normWords.getOrNull(f.start - 1) in indefiniteWords || normWords.getOrNull(f.start - 1) in countWords || normWords.getOrNull(f.start - 1) in setOf("more", "other", "fewer", "choosing", "choose", "chose", "picking", "pick", "picked", "naming", "destroy", "destroys", "exile", "exiles", "all", "no", "any", "nonbasic", "noncreature", "target") || Regex("""^\d+$""").matches(normWords.getOrNull(f.start - 1) ?: ""))) }
            ...
            // "I have Jace Beleren … I activate Jace's +2": a first name already used in full in the question means that
            // card, over the nickname's default (Jace, the Mind Sculptor) as much as over the alternatives.
            .map { f -> if (f.end - f.start == 1 && (f.entry.alternatives.isNotEmpty() || normWords[f.start] in NameIndex.aliases)) short[normWords[f.start]]?.let { f.copy(entry = it) } ?: f else f }
    ...
    private fun readSentence(m: Marked, ctx: Ctx): Boolean {
        ...
        var t2 = t0.replace(Regex("""\balpha[- ]?strikes\b"""), "attacks").replace(Regex("""\balpha[- ]?strike\b"""), "attack")
            ...
            .let { t0 -> Regex("""\b(?:does|will|would|can|could) ((?:my |their |the |his |her )?c\d+) (?:stop|prevent|keep|block) ((?:my |their |his |her )?[^,]+?) from attacking(?: if (i|they|we|he|she|my opponent|the opponent) ([^,]+?))?(?=,| and\b|$)""", RegexOption.IGNORE_CASE).replace(t0) { r ->
                ...
                val mine = r.groupValues[1].startsWith("my ", true) || (r.groupValues[3].lowercase() in setOf("i", "we") && r.groupValues[4].contains("in hand") && !r.groupValues[1].startsWith("their ", true))
                val cond = if (r.groupValues[3].isEmpty()) "" else "${r.groupValues[3]} ${r.groupValues[4]}, "
            ...
            .let { t0 -> Regex("""\b(i|we|they|he|she|my opponent|the opponent) (attacks?|attacking|swings?|swinging) with ((?:my |their |a |an |the )?(?:c\d+|\d+/\d+)) with (a|an|one|two|three|four|five|\d+) ((?:[+-]\d+/[+-]\d+ )?)counters?(?: on it)?\b""", RegexOption.IGNORE_CASE).replace(t0) { r ->
                ...
                val what = r.groupValues[3].replace(Regex("""^(?:my|their) (?=\d+/\d+)""", RegexOption.IGNORE_CASE), "a ")
        ...
        Regex("""\b(?:has|have|with|got|holds?|there is|there are|there's) ((?:an? |two |three |\d+ )?(?:instant|sorcery|sorceries|creature|land|artifact|enchantment|planeswalker|battle)s?(?: cards?)?(?:,? (?:and )?(?:an? |two |three |\d+ )?(?:instant|sorcery|sorceries|creature|land|artifact|enchantment|planeswalker|battle)s?(?: cards?)?)*) (?:is |are )?in (?:(my|their|his|her|the|my opponent's|the opponent's|opponent's) )?graveyards?\b""").find(t2)?.let { r ->
            ...
            val who = when (r.groupValues[2]) { "my", "" -> "me"; "the" -> if (Regex("""\b(?:there is|there are|there's)\s*$""").containsMatchIn(before.trim() + " ") || before.isBlank()) "me" else actorOfClause(t2.trim()) ?: "me"; else -> pronounPlayer(ctx, "their") }
        ...
        Regex("""\b(my|their|his|her) planeswalker\b""").find(t2)?.let { r ->
            ...
            if (pw != null) t2 = t2.replaceRange(r.range, "${r.groupValues[1]} $pw")
        }
        ...
        t2 = t2.replace(Regex("""\b(choosing|picking|selecting) (the )?(\w+)( mode)?,? and (?!(?:i|we|they|he|she|my|their|his|her|the opponent|my opponent|@\w+)\b)(the )?(\w+)( mode)?\b"""), "$1 $2$3$4 & $5$6$7")
        ...
        t2 = t2.replace(Regex("""\b(choosing|picking|selecting) ((?:the )?[a-z][a-z0-9' ]{2,60}?),? and (?!(?:i|we|they|he|she|my|their|his|her|the opponent|my opponent|@\w+)\b)((?:the )?[a-z][a-z0-9' ]{2,60}?)(?=[.,;?]|\s*$)"""), "$1 $2 & $3")
        ...
        Regex("""(?:^|\s+)with ((?:(?:$possPrefix|an? )?c\d+)(?:,? (?:and )?(?:$possPrefix|an? )?c\d+)*) (?:out|on the battlefield|in play|on board|on the field)(?: under (my|their|@\w+'s) control)?$""").find(t2)?.let { r ->
            ...
            if (cards.size > 1 || r.groupValues[2].isNotEmpty() || Regex("""^(?:my opponent's|the opponent's|their|his|her|@\w+'s) """).containsMatchIn(r.groupValues[1])) {
                ...
                // "with my Avacyn and my Bears out": each card keeps the possessive said in front of it.
                val own = Regex("""(my opponent's|the opponent's|my|their|his|her|@\w+'s) (c\d+)""").findAll(r.groupValues[1]).associate { it.groupValues[2] to when (val w = it.groupValues[1]) { "my" -> "me"; "their", "his", "her", "my opponent's", "the opponent's" -> pronounPlayer(ctx, "their"); else -> w.removePrefix("@").removeSuffix("'s") } }
        ...
        val clauses = t.split(clauseSplit).map { it.trim().trimEnd(',', ';').trim() }.filter { it.isNotEmpty() }.fold(mutableListOf<String>()) { acc, cl ->
    ...
    private fun readClause(clauseIn0: String, m: Marked, ctx: Ctx): Boolean {
        ...
        val clauseIn = clauseIn0.replace(Regex("""\b(?:tr(?:y|ies|ied)|attempts?|attempted|want(?:s|ed)?|would like) to (?=(?:activate|use|tap|untap|block|attack|cast|play|sacrifice|equip|counter|draw|search|target|crack|pop|fire|give|put|destroy|exile|bounce|kill|return|regenerate)\b)"""), "")
            ...
            .let { t0 -> Regex("""\b(\d+/\d+) (fliers?|flyers?|tramplers?|deathtouchers?|lifelinkers?|first strikers?|double strikers?)(?=$|[.,;?!])""").replace(t0) { r ->
                r.groupValues[1] + " creature with " + r.groupValues[2].removeSuffix("s").replace("flier", "flying").replace("flyer", "flying").replace("trampler", "trample").replace("deathtoucher", "deathtouch").replace("lifelinker", "lifelink").replace("striker", "strike")
            } }
    ...
    /**
     * Questions answered by asking the engine rather than acting: "do I gain 5?", "how much trample damage goes
     * through?", "can I cast the Bolt this turn?". Read before the generic question handler, which would otherwise
     * read them as actions or swallow them with "answered by the outcome below". Kept apart from readClause0 so
     * that method stays under the JVM's size limit.
     */
    ...
    private fun readEarlyQuestions(clause0: String, m: Marked, ctx: Ctx): Boolean {
        ...
        Regex("""^(?:do|does|will|would) (i|we|they|my opponent|the opponent) (?:still |even )?(?:get|make|create|end up with) (?:an? |the |my |their |any )?(c\d+|tokens?)(?: tokens?)?$""").find(clause0)?.let { r ->
            if (r.groupValues[2].startsWith("token")) {
                // "Do I get a token?": any token that ends up under that player's control.
                val who = if (r.groupValues[1] in setOf("i", "we")) "me" else ctx.other("me") ?: "opp"
                ctx.asks += EventSpec("ask", player = who, to = "tokenMade"); ctx.note(who); ctx.notes += "\"${restore(clause0, m)}?\" is answered by the outcome below."; return true
            }
            val card = m.cards[r.groupValues[2]] ?: return@let
            if (!card.typeLine.startsWith("Token")) return@let
            val who = if (r.groupValues[1] in setOf("i", "we")) "me" else ctx.other("me") ?: "opp"
            if (card.display == "Marit Lage" && ctx.events.none { it.verb == "sacrifice" }) {
                castPermanentObject(ctx)
                val hex = ctx.objects.values.firstOrNull { it.card.name == "Vampire Hexmage" && it.controller == who }?.id
                val depths = ctx.objects.values.firstOrNull { it.card.name == "Dark Depths" && it.controller == who }?.id
                if (hex != null && depths != null) { ctx.events += EventSpec("sacrifice", player = who, obj = hex, targets = listOf(depths)); ctx.notes += "Vampire Hexmage is sacrificed targeting Dark Depths: that is what makes Marit Lage." }
            }
            ctx.asks += EventSpec("ask", player = who, card = CardRef(name = card.display), to = "tokenMade"); ctx.note(who); return true
        }
        // "What's the worst case?" after an attack: their untapped creatures block.
        Regex("""^(?:what(?:'s| is) the worst case(?: scenario)?|worst case(?: scenario)?)$""").find(clause0)?.let {
            val attacks = ctx.events.filter { it.verb == "attack" && it.obj != null }
            val attacker = attacks.firstOrNull()?.player ?: return@let
            val defender = attacks.first().targets.firstOrNull { it in ctx.playerIds() } ?: ctx.other(attacker) ?: return@let
            val blockers = ctx.objects.values.filter { it.controller == defender && it.zone == "battlefield" && isCreatureName(it.card.name) && it.tapped != true && ctx.events.none { e -> e.verb == "block" && e.obj == it.id } }
            if (blockers.isEmpty()) return@let
            blockers.zip(attacks).forEach { (b, a) -> ctx.events += EventSpec("block", player = defender, obj = b.id, targets = listOf(a.obj!!)) }
            ctx.notes += "Worst case read as: ${blockers.zip(attacks).joinToString(", ") { (b, a) -> "their ${b.card.name?.removePrefix("a ")?.removePrefix("an ")} blocks your ${(ctx.objects[a.obj!!]?.card?.name ?: a.obj!!).removePrefix("a ").removePrefix("an ")}" }}."; return true
        }
        // "Does it copy anything?" after a Clone: the trace says what it copied, or that nothing was named.
        Regex("""^(?:does|will|can|did) (?:it|that|(?:the |my )?c\d+) (?:still |even )?copy (?:anything|something|a creature|it|that)$""").find(clause0)?.let { ctx.notes += "\"${restore(clause0, m)}?\" is answered by the outcome below."; return true }
        // "my opponent tries to counter my spell": a plain counterspell at the last spell of the asker's; Vexing Shusher answers it.
        Regex("""^(?:(my opponent|the opponent|opponent|they|he|she|@\w+) (?:tries to |try to |attempts to |wants to |goes to )?counters? (?:my|the|that|this|it) ?(?:spell|creature|it)?|(?:my|the) spell (?:gets|is|would be) countered)$""").find(clause0)?.let { r ->
            val opp = r.groupValues[1].let { if (it.startsWith("@")) it.removePrefix("@") else ctx.other("me") ?: "opp" }
            val myCast = ctx.events.lastOrNull { it.verb == "cast" && it.player == "me" }
            val spellName = myCast?.card?.name ?: myCast?.obj?.let { ctx.objects[it]?.card?.name } ?: run { ctx.events += EventSpec("cast", player = "me", card = CardRef(name = "a spell")); ctx.notes += "No spell of yours was named; a spell stands in."; "a spell" }
            val target = slug(spellName) + ":spell"
            // "My opponent has Mana Drain. They counter it.": the counterspell they were said to hold is the one they use.
            val held = ctx.objects.values.lastOrNull { o -> o.zone == "hand" && o.controller == opp && names.lookup(Names.normalize(o.card.name ?: ""))?.let { needsSpellTarget(it) } == true }
                ?: ctx.inHand[opp]?.lastOrNull { needsSpellTarget(it) }?.let { e -> ctx.inHand[opp]?.remove(e); ctx.objects.getValue(addObject(e, opp, false, ctx, zone = "hand")) }
            if (held != null) {
                ctx.events += EventSpec("cast", player = opp, obj = held.id, targets = listOf(target)); ctx.lastActor = opp; ctx.lastVerb = "cast"; ctx.lastMentioned = held.id
                ctx.notes += "\"Counter it\" is read as casting ${held.card.name} from their hand."; return true
            }
            ctx.events += EventSpec("cast", player = opp, card = CardRef(name = "a counterspell"), targets = listOf(target)); ctx.lastActor = opp; ctx.lastVerb = "cast"
            ctx.notes += "No counterspell was named; assuming a plain \"counter target spell\"."
            // Vexing Shusher: its ability is activated in response, so the counter does nothing.
            ctx.objects.values.firstOrNull { it.controller == "me" && it.zone == "battlefield" && it.card.name == "Vexing Shusher" }?.let { sh ->
                ctx.events += EventSpec("activate", player = "me", obj = sh.id, targets = listOf(target)); ctx.notes += "${sh.card.name}'s ability is activated in response, targeting your spell."
            }
            return true
        }
        ...
        Regex("""^how much (?:(trample)(?: damage)?(?: (?:goes|gets|comes) (?:through|over))?|damage (?:goes|gets|tramples|comes) (?:through|over))(?: to (?:me|them|him|her|my opponent|the opponent|@\w+))?$""").find(clause0)?.let { r ->
        ...
        Regex("""^(?:does|will|would|can) (it|that|(?:my |their |the |his |her )?(c\d+)) (?:still |even )?(?:persist|undying|undie|come back (?:with|via|from|through) (?:its )?(?:persist|undying)(?: counter)?)$""").find(clause0)?.let { r ->
            val id = r.groupValues[2].takeIf { it.isNotEmpty() }?.let { m.cards[it] }?.let { objectIdFor(it, ctx) } ?: ctx.lastMentioned?.takeIf { it in ctx.objects }
    ...
    private fun readCombatStatements(c: String, actor: String?, m: Marked, ctx: Ctx): Boolean {
        // "I flash in Snapcaster Mage and target Lightning Bolt in my graveyard": the target of the spell just cast.
        if (ctx.lastVerb == "cast") Regex("""^(?:targeting|target|aimed at|aiming at|on) (.+)$""").find(c)?.let { r ->
            val idx = ctx.events.indexOfLast { it.verb == "cast" }
            if (idx < 0 || ctx.events[idx].targets.isNotEmpty() || actor != null && actor != ctx.events[idx].player) return@let
            val ev = ctx.events[idx]
            val card = (ev.card?.name ?: ev.obj?.let { ctx.objects[it]?.card?.name })?.let { names.lookup(Names.normalize(it)) } ?: return@let
            // Anything the cast queued after itself (a resolve) is redone by the re-emitted cast.
        ...
        Regex("""^(?:has|have|got) (an? )?(\d+/\d+)(?: creature)?(?: with ($kwPhrase(?:(?:,|,? and) $kwPhrase)*))? (?:is )?blocking (my |their |his |her |the )?(\d+/\d+|c\d+)(?: creature)?$""").find(c)?.let { r ->
            ...
            val attackerOwner = when (r.groupValues[4]) { "my " -> "me"; "their ", "his ", "her " -> pronounPlayer(ctx, "their"); else -> ctx.other(blockerOwner) ?: "me" }
            val attacker = if (Regex("""^c\d+$""").matches(r.groupValues[5])) m.cards[r.groupValues[5]]?.let { objectIdFor(it, ctx) ?: addObject(it, attackerOwner, false, ctx) } ?: return@let
                else describedCreatures("a ", r.groupValues[5], "creature", attackerOwner, ctx, "").firstOrNull() ?: return@let
            val blocker = describedCreatures(r.groupValues[1].ifEmpty { "a " }, r.groupValues[2], "creature", blockerOwner, ctx, r.groupValues[3].split(Regex(""",| and """)).map { it.trim() }.filter { it.isNotEmpty() }.joinToString(", ")).firstOrNull() ?: return@let
            if (ctx.events.none { it.verb == "attack" && it.obj == attacker }) ctx.events += EventSpec("attack", player = attackerOwner, obj = attacker, targets = listOf(blockerOwner))
            ctx.events += EventSpec("block", player = blockerOwner, obj = blocker, targets = listOf(attacker))
            ctx.lastActor = blockerOwner; ctx.lastVerb = "block"; ctx.lastMentioned = blocker; ctx.note(blockerOwner); ctx.note(attackerOwner); return true
        }
    ...
    private fun readClause0(clauseIn: String, m: Marked, ctx: Ctx): Boolean {
        ...
        Regex("""^(?:\s*with (?:$possPrefix|an? )?c\d+(?:,? (?:and )?(?:$possPrefix|an? )?c\d+)*(?: out| on the battlefield| in play| on board| on the field))+$""").find(clauseIn.trim())?.let {
            ...
            for (part in Regex("""with ((?:(?:$possPrefix|an? )?c\d+)(?:,? (?:and )?(?:$possPrefix|an? )?c\d+)*)(?: out| on the battlefield| in play| on board| on the field)""").findAll(clauseIn.trim())) {
                // "they cast Toxic Deluge with my Avacyn out": "my" says whose it is. Read with the clause's actor, Avacyn was theirs.
                val p = part.groupValues[1]
                val said = when { p.startsWith("my ") -> "i have " + p.removePrefix("my "); p.startsWith("their ") || p.startsWith("his ") || p.startsWith("her ") -> "they have " + p.substringAfter(' '); p.startsWith("@") -> p.substringBefore("'s ") + " has " + p.substringAfter("'s "); else -> "have $p" }
                if (readClause("$said out", m, ctx)) read = true
            }
            if (read) return true
        }
        ...
        while (true) {
            val r = Regex("""\s+with (?:an? |the |(my|their|his|her) |@(\w+)'s )?(?:(\d+|two|three|four|five) )?(c\d+) (?:out|in play|on the battlefield|on board|on the field)$""").find(clause0)
                ?.let { r0 -> object { val groupValues = listOf(r0.groupValues[0], r0.groupValues[2], r0.groupValues[3], r0.groupValues[4]); val poss = r0.groupValues[1]; val range = r0.range } } ?: break
            // "they cast Toxic Deluge with my Avacyn out": "my" says whose it is; read with the clause's actor, Avacyn was theirs.
            val owner = r.groupValues[1].ifEmpty { when (r.poss) { "my" -> "me"; "their", "his", "her" -> pronounPlayer(ctx, "their"); else -> actorOfClause(clause0) ?: ctx.lastActor ?: "me" } }
        ...
        Regex("""\s+(?:with|having) (\d+) mana(?: available| up| open| untapped| left)?$""").find(clause0)?.let { r ->
            ...
            setMana(who, r.groupValues[1].toInt(), ctx)
        ...
        Regex("""^(?:how (?:much|many)(?: mana)?(?: more| extra| less)?|what) (?:does|do|would|will|did) ($possPrefix|an? )?(c\d+|it|that|commander) cost(?: to cast)?(?: me| us| them| for me| for them)?(?: to cast)?(?: now| right now| at the moment)?$""").find(clause0)?.let { q ->
            val who = when { q.groupValues[1].startsWith("@") -> q.groupValues[1].removePrefix("@").removeSuffix("'s "); q.groupValues[1] in setOf("their ", "my opponent's ", "the opponent's ", "opponent's ", "an opponent's ", "his ", "her ") -> pronounPlayer(ctx, "their"); else -> "me" }
            ...
            // "I cast Thalia and they cast Lightning Bolt, how much does it cost?": "it" is the spell just cast.
            val lastCast = ctx.events.lastOrNull { it.verb == "cast" }
            // "they cast a 3 mana spell, how much does it cost?": a spell known only by its cost has no card to look up.
            ...
            val card = m.cards[q.groupValues[2]]
                ...
                ?: lastCast?.card?.name?.takeIf { q.groupValues[2] in setOf("it", "that") }?.let { n -> names.lookup(Names.normalize(n)) }
            ...
            val caster = if (q.groupValues[2] in setOf("it", "that") && lastCast?.card?.name == card.display) lastCast.player ?: who else who
        ...
        // "what is it now?" / "what is it in my hand?": the object named last, as it is now. A permanent that left
        // the battlefield is asked what card it is (a Clone is a Clone again); one still there is asked its size.
        Regex("""^what is (?:it|that|the card) now$""").find(clause0)?.let {
            var id = ctx.lastMentioned?.takeIf { it in ctx.objects } ?: castPermanentObject(ctx) ?: return@let
            // "My opponent casts Control Magic on it. What is it now?": the thing the spell was aimed at, not the spell.
            ctx.events.lastOrNull { it.verb == "cast" }?.let { lc -> if ((lc.obj == id || lc.card?.name == ctx.objects.getValue(id).card.name) && lc.targets.any { it in ctx.objects }) id = lc.targets.first { it in ctx.objects } }
            val o = ctx.objects.getValue(id)
            val left = o.zone != "battlefield" || ctx.events.any { e -> e.verb == "leave" && e.obj == id }
            ctx.asks += EventSpec("ask", obj = id, to = if (left || !isCreatureName(o.card.name)) "identity" else "pt"); return true
        }
        ...
        // "Can I put in Avacyn?" after Kaalia attacks: the trigger's choice.
        Regex("""^(?:can i |could i |do i get to |may i )?put in (?:my |an? |the )?(c\d+)(?: with (?:it|her|him|the trigger|(?:the |my )?c\d+(?:'s)?(?: trigger)?))?$""").find(clause0)?.let { r ->
            if (ctx.events.none { it.verb == "attack" || it.verb == "attackAll" }) return@let
            return readClause("put ${r.groupValues[1]} in with the trigger", m, ctx)
        }
        // "I want to keep 2 extra cards" with Sylvan Library: 4 life each.
        Regex("""^(?:i |we )?(?:want to |wants? to |would like to |decide to |choose to )?keep (\d+|one|two|three|both|all three) (?:extra |additional |of the |of those )?cards?(?: (?:with|off|from) (?:it|c\d+))?$""").find(clause0)?.let { r ->
            val lib = m.cards.values.firstOrNull { it.display == "Sylvan Library" } ?: ctx.objects.values.firstOrNull { it.card.name == "Sylvan Library" }?.let { m.cards.values.firstOrNull { c -> c.display == "Sylvan Library" } } ?: return@let
            val n = when (r.groupValues[1]) { "both" -> 2; "all three" -> 3; else -> number(r.groupValues[1]) ?: return@let }
            ctx.asks += EventSpec("ask", to = "text:Keeping $n extra card${if (n == 1) "" else "s"} with ${lib.display} costs ${4 * n} life: for each card drawn this way you keep beyond your normal draw, you pay 4 life or put it back (${lib.display}'s text)."); return true
        }
        ...
        Regex("""^(?:can|could|may|should|is it legal for|am i allowed to|are they allowed to|do|does|will|would|am|is|are) ((?:i|my opponent|the opponent|opponent|they|he|she|we|it|that|@\w+|(?:my |the |their |his |her |an? )?(?:c\d+|\d+/\d+(?: [a-z]+)*|creature|token|guy|attacker|blocker|dude|beater))\b.*)$""").find(clause0)?.let { r ->
            ...
            // "Can I counter it?" / "can I save it?" / "can I sacrifice it first?": answers to the spell, so it stays on the stack.
            val answersTheStack = Regex("""\b(?:in response|respond|responding|counter|counters|save|protect|rescue|stifle|redirect|fizzle|sacrifice it|sac it|first|before it resolves|stop it|stop that|stop the spell|prevent it|answer it|deal with it|do anything about it)\b""").containsMatchIn(r.groupValues[1]) ||
                Regex("""c\d+""").findAll(r.groupValues[1]).any { ph -> m.cards[ph.value]?.let { c -> needsSpellTarget(c) || c.display in setOf("Stifle", "Trickbind", "Tale's End", "Disallow", "Voidslime") } == true }
            if (!answersTheStack && ctx.events.lastOrNull()?.verb in setOf("cast", "activate", "trigger")) ctx.events += EventSpec("resolveAll")
            ...
            Regex("""^(?:it|that|(?:my |the |(their |his |her ))?(c\d+|\d+/\d+(?: [a-z]+)*)) (?:still |even )?attacks?(?: (?:this|next) turn| now| right away| at all)?$""").find(r.groupValues[1])?.let { q0 ->
                // "can their 0/1 attack?": the creature is the opponent's, so the attack is theirs.
                val theirs = q0.groupValues[1].isNotEmpty()
                ...
                val attacker = subjId?.let { ctx.objects.getValue(it).controller } ?: if (theirs) pronounPlayer(ctx, "their") else "me"
        ...
        // "they attack with two creatures with only 2 lands untapped": how much mana that player has, said at the
        // end of what they do. Left on the clause, the whole attack went unread.
        Regex("""\s+(?:with|having|and(?: (?:they|i))?(?: only)?(?: have| has)?)(?: only| just)? (\d+|one|two|three|four|five|six|seven|eight|nine|ten|no|zero) (?:lands?|mana|mana sources?)(?: untapped| open| up| available| left)?$""").find(clause0)?.let { r ->
            if (Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(?:only )?(?:has|have|with|got)\b""").containsMatchIn(clause0)) return@let
            val who = actorOfClause(clause0) ?: ctx.lastActor ?: "me"
            val n = if (r.groupValues[1] in setOf("no", "zero")) 0 else number(r.groupValues[1]) ?: return@let
            // "they cast Ponder with no mana open": nothing left over once it is cast, so no optional payment is made.
        ...
        // "I kill it in response": a trailing "in response" on a plain removal says when, and left there it stopped
        // the clause matching, so the kill silently vanished.
        if (Regex("""^(?:(?:i|we|they|he|she) )?(?:kills?|destroys?|exiles?|bounces?|removes?|nukes?|sacrifices?|sacs?) .+ in response(?: to (?:it|that|the (?:spell|ability|trigger|activation)))?$""").matches(clause0)) clause0 = clause0.replace(Regex("""\s+in response(?: to (?:it|that|the (?:spell|ability|trigger|activation)))?$"""), "")
        ...
        // "I cast Thoughtseize and take their Counterspell": what a discard spell takes is discarded, not stolen. Read
        // as taking control, the Counterspell was put on the battlefield under the asker's control.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(?:takes?|took|taking|snags?|snagged|grabs?|grabbed|picks?|picked|strips?|stripped|nabs?|nabbed) (?:their|his|her|the|my opponent's|my|@\w+'s) (c\d+)(?: (?:with|to|from (?:their|his|her|my) hand)(?: it)?)?$""").find(c)?.let { r ->
            val card = m.cards.getValue(r.groupValues[1])
            val lastCast = ctx.events.lastOrNull { it.verb == "cast" }
            val discardSpell = lastCast?.card?.name?.lowercase() in discardSpells
            if (!discardSpell && !card.isSpellOnly) return@let
            val taker = actorOfClause(c) ?: lastCast?.player ?: ctx.lastActor ?: "me"
            val victim = if (c.startsWith("my ") || Regex("""^(?:i|we) """).containsMatchIn(c) && r.groupValues[0].contains(" my ")) taker else ctx.other(taker) ?: "opp"
            val id = ctx.objects.values.firstOrNull { it.card.oracleId == card.oracleId && it.zone == "hand" && it.controller == victim }?.id
                ?: addObject(card, victim, false, ctx, zone = "hand", allowDuplicate = true)
            ctx.objects[id] = ctx.objects.getValue(id).copy(zone = "hand")
            ctx.events += EventSpec("discard", player = victim, obj = id)
            ctx.lastActor = taker; ctx.lastVerb = "discard"; ctx.lastMentioned = id; ctx.note(victim); return true
        }
        // "I put in a Snapcaster at instant speed", "vial in Snapcaster": the creature comes in off Aether Vial.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(?:vials? in|puts? in|drops? in|vials?) (?:an? |my |the )?(c\d+)(?: (?:with|off|using|via) (?:my |the |their )?(c\d+))?(?: at instant speed| on (?:their|my) turn| in response| at (?:their |my )?(?:end step|eot|end of turn)| during combat)?$""").find(c)?.let { r ->
            val who = actorOfClause(c) ?: ctx.lastActor ?: "me"
            val vial = r.groupValues[2].takeIf { it.isNotEmpty() }?.let { m.cards.getValue(it) }?.let { objectIdFor(it, ctx) ?: addObject(it, who, false, ctx) }
                ?: ctx.objects.values.lastOrNull { it.controller == who && it.zone == "battlefield" && it.card.name in setOf("Aether Vial", "Stoneforge Mystic") }?.id ?: return@let
            val card = m.cards.getValue(r.groupValues[1])
            val cardId = addObject(card, who, false, ctx, zone = "hand", allowDuplicate = true)
            ctx.events += EventSpec("activate", player = who, obj = vial, to = "put:$cardId"); ctx.lastActor = who; ctx.lastMentioned = cardId; return true
        }
        // "I play a land and Sol Ring turn 1": the second thing played, with the verb left off and the turn said after it.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(c\d+)(?: on)? (?:turn|t) ?(\d+|one|two|three|four|five)$""").find(c)?.let { r ->
            val card = m.cards.getValue(r.groupValues[1])
            val subj = actorOfClause(c) ?: ctx.lastActor ?: "me"
            val says = if (subj == "me") "i " else if (subj == "opp") "they " else "@$subj "
            return readClause(says + (if (card.typeLine.contains("Land")) "play " else "cast ") + r.groupValues[1], m, ctx)
        }
        // "my commander dies" / "their commander gets exiled": the commander, whichever card it is, leaves.
        Regex("""^(my|their|his|her|my opponent's|the opponent's|@\w+'s) commander (dies|died|is destroyed|gets destroyed|is killed|gets killed|is exiled|gets exiled|is bounced|gets bounced|goes to the graveyard)$""").find(c)?.let { r ->
            ...
            val id = commanderObject(who, ctx)
        ...
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(?:chump[- ]?)?blocks?(?: it| that| the attacker)? with (?:an? |their |the |my )?((?:\d+/\d+ )?(?:[a-z]+ )?token)$""").find(c)?.let { r ->
            ...
            val attacker = ctx.events.lastOrNull { it.verb == "attack" && who in it.targets }?.obj ?: ctx.events.lastOrNull { it.verb == "attack" }?.obj ?: return@let
            ...
            if (name == "1/1 creature token") ctx.notes += "The token's stats weren't given; assuming a 1/1 creature token."
            ctx.events += EventSpec("block", player = who, obj = id, targets = listOf(attacker)); ctx.lastVerb = "block"; ctx.lastActor = who; ctx.lastMentioned = id; return true
        }
        // "I kill the blocker", "they bounce the attacker": the creature in that combat role leaves, with no card named.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(kills?|killed|destroys?|destroyed|exiles?|exiled|bounces?|bounced|removes?|removed) (?:the |their |my |his |her )?(blocker|blocking creature|chump blocker|attacker|attacking creature)(?: before (?:combat )?damage| in response| first)?$""").find(c)?.let { r ->
            val id = (if (r.groupValues[2].startsWith("attack")) ctx.events.lastOrNull { it.verb == "attack" }?.obj else ctx.events.lastOrNull { it.verb == "block" }?.obj) ?: return@let
            val zone = when { r.groupValues[1].startsWith("exile") -> "exile"; r.groupValues[1].startsWith("bounce") -> "hand"; else -> "graveyard" }
            ctx.events += EventSpec("leave", obj = id, to = zone); ctx.lastMentioned = id; return true
        }
        // "do they pay?" / "do I have to pay?": whether to pay is that player's choice; the answer says what happens if they don't.
        Regex("""^(?:do|does|will|would|must|should) (they|i|we|he|she|my opponent|the opponent) (?:have to |need to |get to )?pay(?: for it| it| the tax| the \d+| \d+| the mana| the one)?$""").find(c)?.let { r ->
            ctx.notes += "Whether ${if (r.groupValues[1] in setOf("i", "we")) "you pay" else "they pay"} is a choice made as the ability resolves (they may pay if they have the mana); the outcome below shows what happens if the payment isn't made."
            return true
        }
        // "+1 my opponent's Grave Titan" (Oko): the loyalty ability named by its cost, aimed at something that isn't the planeswalker.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?([+\u2212-]\d+) (?:targeting |on |at )?(my opponent's|the opponent's|their|his|her|my|the)? ?(c\d+)$""").find(c)?.let { r ->
            val target = m.cards.getValue(r.groupValues[3])
            if (target.typeLine.contains("Planeswalker")) return@let
            val who = actorOfClause(c) ?: ctx.lastActor ?: "me"
            val walker = castPermanentObject(ctx)?.takeIf { id -> ctx.objects[id]?.card?.name?.let { n -> names.lookup(Names.normalize(n))?.typeLine?.contains("Planeswalker") } == true }
                ?: ctx.objects.values.lastOrNull { o -> o.controller == who && o.zone == "battlefield" && o.card.name?.let { n -> names.lookup(Names.normalize(n))?.typeLine?.contains("Planeswalker") } == true }?.id ?: return@let
            val owner = when (r.groupValues[2]) { "my" -> who; "", "the" -> null; else -> ctx.other(who) ?: "opp" }
            val tid = objectIdFor(target, ctx) ?: addObject(target, owner ?: ctx.other(who) ?: "opp", false, ctx)
            ctx.events += EventSpec("activate", player = who, obj = walker, to = r.groupValues[1].replace('\u2212', '-'), targets = listOf(tid)); ctx.lastActor = who; ctx.lastMentioned = tid; return true
        }
        // "I +2 Jace" / "+1 Liliana": the loyalty ability named by its cost, said before the planeswalker.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?([+\u2212-]\d+) (?:with |on |my |the |their )*(c\d+)(.*)$""").find(c)?.let { r ->
            val who = actorOfClause(c) ?: ctx.lastActor ?: "me"
            ...
            ctx.events += EventSpec("activate", player = who, obj = id, to = r.groupValues[1].replace('\u2212', '-'), targets = targetsIn(r.groupValues[3], m, ctx)); ctx.lastActor = who; ctx.lastMentioned = id; return true
        }
        // "they cast Liliana and +1": a bare loyalty cost is that player's planeswalker's ability.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?([+\u2212-]\d+)(?: (?:it|her|him))?(.*)$""").find(c)?.let { r ->
            val who = actorOfClause(c) ?: ctx.lastActor ?: "me"
            val pw = ctx.objects.values.lastOrNull { o -> o.controller == who && (o.card.name == "a planeswalker" || names.lookup(Names.normalize(o.card.name ?: ""))?.typeLine?.contains("Planeswalker", true) == true) }?.id
                ?: ctx.events.lastOrNull { it.verb == "cast" && it.player == who }?.card?.name?.let { n -> names.lookup(Names.normalize(n)) }?.takeIf { it.typeLine.contains("Planeswalker") }?.let { linkCastToObject(it, who, ctx) }
                ?: return@let
            ctx.events += EventSpec("activate", player = who, obj = pw, to = r.groupValues[1].replace('\u2212', '-'), targets = targetsIn(r.groupValues[2], m, ctx)); ctx.lastActor = who; ctx.lastMentioned = pw; return true
        }
        // "put a counter on my creature" with no creature described: one stands in.
        Regex("""^(?:(?:i|we) )?puts? (an? |one |two |three |\d+ )?([+-]\d+/[+-]\d+ |[a-z]+ )?counters? on my (creature|guy|dude)$""").find(c)?.let { r ->
            if (ctx.objects.values.any { it.controller == "me" && it.zone == "battlefield" && isCreatureName(it.card.name) }) return@let
            if (!readClause("i have a creature", m, ctx)) return@let
            ctx.notes += "No creature of yours was described; a creature stands in for it."
            return readClause("put ${r.groupValues[1].ifEmpty { "a " }}${r.groupValues[2]}counter on it", m, ctx)
        }
        // "Urza's Saga on chapter 2": a Saga with that many lore counters.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(?:(?:have|has|got|with|control|controls) )?(?:an? |the |my |their )?(c\d+) (?:on|at|is on|is at|in) chapter (\d+|one|two|three|four|five)$""").find(c)?.let { r ->
            val card = m.cards.getValue(r.groupValues[1])
            if (!card.typeLine.contains("Saga")) return@let
            val who = actorOfClause(c) ?: ctx.lastActor ?: "me"
            val n = number(r.groupValues[2]) ?: return@let
            val id = objectIdFor(card, ctx) ?: addObject(card, who, false, ctx)
            ctx.objects[id] = ctx.objects.getValue(id).copy(counters = ctx.objects.getValue(id).counters + ("lore" to n))
            ctx.lastMentioned = id; ctx.note(who); return true
        }
        // "Chalice of the Void on 1", "chalice at 2", "Engineered Explosives with 3 counters": an artifact set to a number.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(?:(?:have|has|got|with|controls?) )?(?:an? |the |my |their )?(c\d+) (?:on|at|set to|set at|for|with|is on|is at|is set to) (\d+|zero|one|two|three|four|five)(?: (?:charge )?counters?)?$""").find(c)?.let { r ->
            val card = m.cards.getValue(r.groupValues[1])
            if (!card.typeLine.contains("Artifact") || card.typeLine.contains("Creature")) return@let
            val who = actorOfClause(c) ?: ctx.lastActor ?: "me"
            val n = if (r.groupValues[2] == "zero") 0 else number(r.groupValues[2]) ?: return@let
            val id = objectIdFor(card, ctx) ?: addObject(card, who, false, ctx)
            ctx.objects[id] = ctx.objects.getValue(id).copy(counters = ctx.objects.getValue(id).counters + ("charge" to n))
            ctx.lastMentioned = id; ctx.lastVerb = "have"; ctx.lastOwner = who; ctx.note(who); return true
        }
        // "Kitchen Finks with a -1/-1 counter dies": the counter is a fact about it, and then it dies.
        Regex("""^((?:my |their |the )?c\d+|it|that) (?:with|that has|which has|having) (an? |one |two |three |\d+ )?(?:([+-]\d+/[+-]\d+|[a-z]+) )?counters?(?: on it)? (dies|is destroyed|gets destroyed|is killed|gets killed|is sacrificed|gets sacrificed|is exiled|gets exiled|is bounced|gets bounced|blocks|attacks)$""").find(c)?.let { r ->
            if (!readClause("${r.groupValues[1]} has ${r.groupValues[2].ifEmpty { "a " }}${r.groupValues[3].let { if (it.isEmpty()) "" else "$it " }}counter", m, ctx)) return@let
            return readClause("${r.groupValues[1]} ${r.groupValues[4]}", m, ctx)
        }
        // "I have 4 lands and cast Cryptic Command, can I?": whether the spell just cast can be paid for.
        ...
        Regex("""^(?:do|will|would|does) (?:all )?(?:their|his|her|my opponent's|the opponent's) (creatures|guys|dudes|team|board|stuff|tokens) (?:all )?(?:die|survive|live|get destroyed|get wiped|get killed|get bounced|make it|be safe|be fine|go away)$""").find(c)?.let { r ->
            ...
            if (!had) {
                ctx.notes += "No ${if (tokens) "token" else "creature"} of theirs was described; ${if (tokens) "a 1/1 token" else "a creature"} stands in for them so the answer says what happens to one."
                if (!readClause(if (tokens) "they have a 1/1 token" else "they have a creature", m, ctx)) return@let
            }
        ...
        Regex("""^(?:do|will|would|does) (?:all )?my (creatures|guys|dudes|team|board|stuff|tokens) (?:all )?(?:die|survive|live|get destroyed|get wiped|get killed|get bounced|make it|be safe|be fine|come back|go away)$""").find(c)?.let { r ->
            ...
            if (!hadCreature) {
                ctx.notes += "No ${if (tokens) "token" else "creature"} of yours was described; ${if (tokens) "a 1/1 token" else "a creature"} stands in for them so the answer says what happens to one."
                if (!readClause(if (tokens) "i have a 1/1 token" else "i have a creature", m, ctx)) return@let
            }
            // "I have Heroic Intervention and they cast Damnation, do my creatures die?": the instant in hand is the
            // answer the asker is holding, so it is cast in response — and the answer says so.
            val held = ctx.objects.values.lastOrNull { it.controller == "me" && it.zone == "hand" && names.lookup(Names.normalize(it.card.name ?: ""))?.isSpellOnly == true }
            val theirCast = ctx.events.lastOrNull()?.takeIf { it.verb == "cast" && it.player != "me" }
            if (held != null && theirCast != null) {
                val card = names.lookup(Names.normalize(held.card.name ?: "")) ?: return true
                ctx.notes += "${card.display} is in your hand; assuming you cast it in response."
                emitCast("me", card, "", m, ctx)
            }
            // "They cast Wrath. I have Selfless Spirit. Do my creatures survive?": anything the asker has that would
            // protect the team is used in response, quietly when there is nothing.
            if (theirCast != null) ctx.objects.values.firstOrNull { it.controller == "me" && it.zone == "battlefield" && isCreatureName(it.card.name) && (!tokens || it.token) }?.let { first -> ctx.events += EventSpec("save", player = "me", obj = first.id, to = "quiet") }
            // The question itself: each of the asker's creatures is asked about once everything has played out.
            for (o in ctx.objects.values.filter { it.controller == "me" && it.zone == "battlefield" && isCreatureName(it.card.name) && (!tokens || it.token) }) ctx.asks += EventSpec("ask", obj = o.id, to = "die")
            return true
        }
        // "I flash in a Snapcaster to block": a named creature cast now, and it blocks.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(?:flash(?:es|ed)? in|flashes) (?:an? |my |the )?(c\d+)( (?:to|and) (?:blocks?|chumps?)(?: (?:it|the attacker|with it))?| as a (?:chump )?blocker| to chump)?$""").find(c)?.let { r ->
            val who = actorOfClause(c) ?: ctx.lastActor ?: "me"
            val card = m.cards.getValue(r.groupValues[1])
            val id = addObject(card, who, false, ctx, zone = "hand", allowDuplicate = true)
            ctx.events += EventSpec("cast", player = who, obj = id)
            if (r.groupValues[2].isNotEmpty()) {
                ...
                ctx.events += EventSpec("resolveAll"); ctx.events += EventSpec("block", player = who, obj = id, targets = listOfNotNull(attacker)); ctx.lastVerb = "block"
            } else ctx.lastVerb = "cast"
            ctx.lastActor = who; ctx.lastMentioned = id; return true
        }
        // "flashback it at their 2/2", "I flash back Lightning Bolt targeting X": cast from the graveyard with flashback.
        Regex("""^(?:(?:i|we|they|he|she|my opponent|the opponent|@\w+) )?(?:flash(?:es|ed)? ?back|flashbacks?) (?:it|that|(?:the |my |their )?(c\d+))((?: (?:at|on|targeting)\b.*)?)$""").find(c)?.let { r ->
            val who = actorOfClause(c) ?: ctx.lastActor ?: "me"
            val card = m.cards[r.groupValues[1]]
                ?: (ctx.lastMentioned?.let { ctx.objects[it] }?.takeIf { it.zone == "graveyard" } ?: ctx.objects.values.lastOrNull { it.zone == "graveyard" && it.controller == who })
                    ?.card?.name?.let { n -> names.lookup(Names.normalize(n)) } ?: return@let
            emitCast(who, card, r.groupValues[2] + " with flashback", m, ctx); return true
        }
        ...
        Regex("""^(?:(my|their|his|her|my opponent's|the opponent's|opponent's|@\w+'s|the) )?(?:(c\d+|it|that|they)(?:'s)? )?(?:(?:the|its|his|her|their) )?(?:triggered )?(?:ability |abilities |trigger |triggered ability )?(?:triggers?|triggered|goes off|go off|went off|fires?|fired|is on the stack|is waiting to resolve|is triggering)$""").find(c)?.let { r ->
            // "I Stifle the trigger": an instant's name used as a verb, aimed at the trigger — not the instant triggering.
            if (r.groupValues[2].takeIf { cardRef.matches(it) }?.let { m.cards[it] }?.isSpellOnly == true) return@let
            if (ctx.events.isNotEmpty()) {
                ...
                r.groupValues[2].takeIf { cardRef.matches(it) }?.let { ph -> m.cards[ph] }
            ...
            val ph = r.groupValues[2]
            // "My opponent's Blood Artist trigger is on the stack": whose permanent it is, said in the clause.
            val owner = when (val w = r.groupValues[1]) { "" , "the" -> actor ?: ctx.lastOwner; "my" -> "me"; "their", "his", "her", "my opponent's", "the opponent's", "opponent's" -> pronounPlayer(ctx, "their"); else -> w.removePrefix("@").removeSuffix("'s") }
            val id = (if (cardRef.matches(ph)) m.cards[ph]?.let { card -> objectIdFor(card, ctx) ?: addObject(card, owner, false, ctx) }
        ...
        Regex("""^(?:$activateVerbs)\s+(?:it|that|him|her|them|its ability|his ability|her ability|it's ability)\b(?!\s*[+\u2212-]?\d)(?!\s+(?:choosing|naming|picking|for|on) (?:white|blue|black|red|green|colou?rless)\b)(.*)$""").find(c)?.let { r ->
            ...
            val id = ctx.lastMentioned?.takeIf { it in ctx.objects && ctx.objects.getValue(it).controller == who }
                ...
                // "I cast Walking Ballista for X=3 and activate it": the permanent just cast, which the cast alone didn't put on the board.
                ?: ctx.events.lastOrNull { it.verb == "cast" && it.player == who }?.card?.name?.let { n -> names.lookup(Names.normalize(n)) }?.takeIf { !it.isSpellOnly }?.let { linkCastToObject(it, who, ctx) }
            ...
            val xValue = xRe.find(r.groupValues[1])?.let { x -> (x.groupValues[1].ifEmpty { x.groupValues[2] }).toIntOrNull() }
            val targets = targetsIn(r.groupValues[1].replace(timesRe, "").replace(xRe, "").let { s -> colour?.let { s.replace(colourRe, "") } ?: s }.trim(), m, ctx).ifEmpty { if (colour != null) listOf(id) else emptyList() }
            repeat(times) { ctx.events += EventSpec("activate", player = who, obj = id, targets = targets, to = colour?.let { "color:$it" }, amount = xValue) }
        ...
        Regex("""^(?:sacrifices?|sacs?|sacrificing|saccing) (?:an? |one |another )(land|creature|artifact|enchantment|permanent|token)(?: (?:to|into|with) (?:it|that|(?:my |the )?(c\d+))(?:'s ability)?)?(?: to (?:protect|save|shroud|shield) (?:it|that|them|(?:my |the )?(c\d+)))?$""").find(c)?.let { r ->
            ...
            if (kind == "creature" && ctx.objects.values.none { it.controller == who && it.id != outlet && it.zone == "battlefield" && isCreatureName(it.card.name) }) {
                describedCreatures("a ", "", "creature", who, ctx, "")
                ctx.notes += "No creature was described besides ${ctx.objects[outlet]?.card?.name ?: "the outlet"}, so one is taken as read for the sacrifice."
            }
        ...
        // "I sacrifice Kitchen Finks with a -1/-1 counter": the counter is a fact about it before it goes.
        Regex("""^(sacrifices?|sacs?|sacrificing|saccing) ((?:an? |the |my )?c\d+) with ((?:an? |one |two |three |\d+ )?[+-]\d+/[+-]\d+ counters?(?: on it)?)(.*)$""").find(c)?.let { r ->
            val card = m.cards[r.groupValues[2].substringAfterLast(' ')] ?: return@let
            val who = actor ?: subject ?: "me"
            val id = objectIdFor(card, ctx) ?: addObject(card, who, false, ctx)
        ...
        // "my 1/1 with Rancor attacking" / "a 2/2 equipped with Bonesplitter is attacking": the creature is described,
        // its Aura or Equipment attached, and it attacks.
        Regex("""^(?:(?:have|has|got|controls?)\s+)?(?:(my opponent's|the opponent's|their|his|her|my|the|@\w+'s) )?(an? )?(\d+/\d+)(?: ($creatureKinds))? (?:with|wearing|carrying|equipped with|enchanted with) (?:an? |the |my )?(c\d+)(?: attached| on it)? (?:is |are )?attacking(?: (me|them|him|her|my opponent|the opponent|@\w+))?$""").find(c)?.let { r ->
            val eqCard = m.cards[r.groupValues[5]] ?: return@let
            if (!eqCard.typeLine.contains("Equipment") && !eqCard.typeLine.contains("Aura")) return@let
            val head = r.groupValues[1]
            val who = when { head.startsWith("my opponent") || head.startsWith("the opponent") || head in setOf("their", "his", "her") -> pronounPlayer(ctx, "their"); head == "my" -> "me"; head.startsWith("@") -> head.removePrefix("@").removeSuffix("'s"); else -> actor ?: ctx.lastOwner ?: "me" }
            val host = describedCreatures(r.groupValues[2].ifEmpty { "a " }, r.groupValues[3], r.groupValues[4].ifEmpty { "creature" }, who, ctx, "").firstOrNull() ?: return@let
            val eq = objectIdFor(eqCard, ctx) ?: addObject(eqCard, who, false, ctx)
            ctx.objects[eq] = ctx.objects.getValue(eq).copy(attachedTo = host)
            val defender = r.groupValues[6].let { d -> when { d.isEmpty() -> ctx.other(who) ?: "opp"; d.startsWith("@") -> d.removePrefix("@"); d == "me" -> "me"; else -> pronounPlayer(ctx, d.substringAfterLast(' ')) } }
            ctx.events += EventSpec("attack", player = who, obj = host, targets = listOf(defender))
            ctx.lastVerb = "attack"; ctx.lastActor = who; ctx.lastOwner = who; ctx.lastMentioned = host; ctx.note(who); ctx.note(defender); return true
        }
        ...
        if (readCombatStatements(c, actor, m, ctx)) return true
        ...
        Regex("""^(?:(?:have|has|got|control|controls|controlling|'ve got)\s+)?(an? |one |\d+ |two |three |four |five )?(?:other |more )?(tapped |untapped )?(?:(\d+/\d+)s?\s*)?((?:first strike |double strike |(?!with )[a-z]+ )*)($creatureKinds|blockers?|attackers?|guys?|dudes?|beaters?|bodies|body|$kwNouns)?(?: with ($kwPhrase(?:(?:,|,? and|,? &) $kwPhrase)*|(?:an? |one |two |three |four |five |\d+ )?[+-]\d+/[+-]\d+ counters?(?: on it)?))?(?: plus .*)?(?: on the battlefield| in play| out)?((?: that (?:i|they|he|she) just (?:played|cast)(?: this turn)?| that just came down| i just played| with summoning sickness| that has been out| from last turn)?)$""").find(c)?.let { r ->
            ...
            Regex(""" plus (an? |\d+ |two |three |four |five )?(\d+/\d+)s?(?: ($kwNouns))?(?: ($creatureKinds))?(?: with ($kwPhrase(?:(?:,|,? and) $kwPhrase)*))?""").findAll(c).forEach { x ->
                // "a 1/1 with infect plus a 1/1 with wither": the second creature keeps its keywords too.
                describedCreatures(x.groupValues[1], x.groupValues[2], x.groupValues[4], who, ctx, listOf(x.groupValues[3].let { k -> if (k.isEmpty()) "" else k.removeSuffix("s").replace("flier", "flying").replace("flyer", "flying").replace("trampler", "trample") }, x.groupValues[5].replace(Regex(""",? and """), ", ")).filter { it.isNotEmpty() }.joinToString(", "))
            }
        ...
        Regex("""^(?:have|has|got|control|controls|controlling|'ve got|make|makes|made|create|creates|created)\s+(an? |\d+ |two |three |four |five )?((?:\d+/\d+ )?(?:(?:white|blue|black|red|green|colorless) )*(?:[a-z]+ )*?(?:creature |artifact )?tokens?)(.*)$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            val n = r.groupValues[1].trim().let { if (it.isEmpty()) 1 else number(it) ?: 1 }
            val name = r.groupValues[2].trim().removeSuffix("s").let { if (it.endsWith(" token")) it else "$it token" }.let { if (it == "token token" || it == "token") "1/1 creature token" else it }
    ...
    private fun readClause1(c0: String, clause0: String, actor: String?, subject: String?, clauseIn: String, m: Marked, ctx: Ctx): Boolean {
        ...
        Regex("""^($possPrefix)?(c\d+|it|that|creature|guy|dude|\d+/\d+) (gets?|has|have|is given|gains?|gained) (an?|one|two|three|four|five|\d+|another)? ?(?:(?:more |additional )?([+-]\d+/[+-]\d+|[a-z]+) )?counters?(?: on it)?$""").find(c)?.let { r ->
            ...
            val n = r.groupValues[4].ifEmpty { "one" }.let { number(it) } ?: return@let
            ...
            val kind = r.groupValues[5].ifEmpty {
            ...
            // "my Bears gets a +1/+1 counter": something happening, not a description of the board — so Doubling
            // Season and Hardened Scales see it. "Has" / "with" stays a fact about the permanent.
            if (r.groupValues[3].startsWith("get") || r.groupValues[3].startsWith("gain") || r.groupValues[3] == "is given") {
                ctx.events += EventSpec("counters", obj = id, amount = n, to = kind)
        ...
        Regex("""^(?:puts?|put|adds?|added|places?|placed) (an?|one|two|three|four|five|\d+|some|more)? ?(?:([+-]\d+/[+-]\d+|[a-z]+) )?counters? (?:on|onto|to) (?:(?:my |their |the |his |her )?(c\d+)|(?:(my|their|the|his|her) )?(\d+/\d+|creature|guy|dude|token)(?: creature)?|it|that|itself)$""").find(c)?.let { r ->
            ...
            // "put a counter on my Bears": the kind left unsaid is +1/+1, the counter creatures get by far the most often.
            val kind = r.groupValues[2].ifEmpty { ctx.notes += "\"a counter\" is read as a +1/+1 counter; say the kind if you meant another."; "+1/+1" }
        ...
        // "exile a land from my opponent's graveyard for mana" (Deathrite Shaman): the ability of the permanent that
        // does that, aimed at such a card, which is taken as read in that graveyard.
        Regex("""^exiles? an? (land|instant|sorcery|creature)(?: card)? from ($possPrefix|an? )?graveyards?(?: (?:for|to make|to add|for the) mana| with (?:my |the )?(c\d+))?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            val src = r.groupValues[3].takeIf { it.isNotEmpty() }?.let { m.cards[it] }?.let { objectIdFor(it, ctx) ?: addObject(it, who, false, ctx) }
            ...
            val owner = possessiveOwner(r.groupValues[2].takeIf { it != "a " && it != "an " } ?: "", ctx, m) ?: ctx.other(who) ?: "opp"
            val kind = r.groupValues[1]
            var id = slug("a $kind card"); var k = 2; while (ctx.objects.containsKey(id)) id = slug("a $kind card") + "_" + (k++)
            ctx.objects[id] = ObjectSpec(id, CardRef(name = if (kind == "land") "a basic land" else "an $kind card".replace("an sorcery", "a sorcery").replace("an creature", "a creature").replace("an land", "a land")), controller = owner, zone = "graveyard")
            ctx.events += EventSpec("activate", player = who, obj = src, targets = listOf(id)); ctx.lastActor = who; ctx.lastMentioned = src; ctx.note(owner); return true
        }
        // "I block both" / "block both of them with my 4/4": one creature declared on every attacker; the engine
        // allows the first and refuses the rest (509.1a), which is the answer.
        Regex("""^blocks? (?:both|both of them|all of them|each of them|all|everything|all attackers|both attackers)(?: with (?:my |the )?(c\d+|\d+/\d+|creature|guy|dude))?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            val attackers = ctx.events.filter { it.verb == "attack" && it.player != who }.mapNotNull { it.obj }
            if (attackers.size < 2) return@let
            val b = r.groupValues[1]
            val blocker = when {
                b.isEmpty() -> ctx.objects.values.lastOrNull { it.controller == who && it.zone == "battlefield" && isCreatureName(it.card.name) && it.id !in attackers }?.id
                Regex("""^c\d+$""").matches(b) -> m.cards[b]?.let { objectIdFor(it, ctx) ?: addObject(it, who, false, ctx) }
                Regex("""^\d+/\d+$""").matches(b) -> describedCreatures("the ", b, "creature", who, ctx).firstOrNull()
                else -> ctx.objects.values.lastOrNull { it.controller == who && it.zone == "battlefield" && isCreatureName(it.card.name) && it.id !in attackers }?.id
            } ?: return@let
        ...
        // "cast Snapcaster's flashback spell" / "flash back a spell": an instant with flashback stands in for it.
        Regex("""^(?:casts?|flashbacks?|flash(?:es)? back) (?:(?:my |the )?c\d+'s flashback spell|(?:a|the|my) (?:flashback spell|spell with flashback|card with flashback|flashback card))(?: from (?:my|the|their) graveyard)?$|^flashbacks? (?:a|the|their|my) (?:spell|card|instant|sorcery)(?: from (?:my|the|their) graveyard)?$""").find(c)?.let {
            val who = actor ?: subject ?: "me"
            ctx.events += EventSpec("cast", player = who, card = CardRef(name = "a flashback spell"), to = "flashback"); ctx.lastActor = who; ctx.lastVerb = "cast"
            ctx.notes += "No flashback spell was named; an instant with flashback stands in for it."; return true
        }
        // "Alice blocks the one attacking her (with her 2/2)": the attacker aimed at that player is what's blocked.
        Regex("""^(?:chump[- ]?)?(?:blocks?|blocking) (?:the )?(?:one|creature|attacker|guy|dude) (?:that is |that's |who is )?attacking (me|us|her|him|them|@\w+|my opponent)(?: with (?:my |her |his |their |the |an? )?(c\d+|\d+/\d+(?: [a-z]+)?))?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            val victim = when (val w = r.groupValues[1]) { "me", "us" -> "me"; "my opponent" -> pronounPlayer(ctx, "opponent"); "her", "him", "them" -> who; else -> w.removePrefix("@") }
            val attacker = ctx.events.lastOrNull { it.verb == "attack" && it.targets.firstOrNull() == victim }?.obj ?: return@let
            val b = r.groupValues[2]
            val blocker = when {
                b.isEmpty() -> ctx.objects.values.lastOrNull { it.controller == who && it.zone == "battlefield" && isCreatureName(it.card.name) && it.id != attacker }?.id
                    ?: describedCreatures("a ", "", "creature", who, ctx, "").firstOrNull()?.also { ctx.notes += "${ctx.players[who] ?: who} blocks with a creature nobody described; a 1/1 stands in." }
                Regex("""^c\d+$""").matches(b) -> m.cards[b]?.let { objectIdFor(it, ctx) ?: addObject(it, who, false, ctx) }
                else -> describedCreatures("the ", b.substringBefore(' '), "creature", who, ctx, b.substringAfter(' ', "")).firstOrNull()
            } ?: return@let
            ctx.events += EventSpec("block", player = who, obj = blocker, targets = listOf(attacker)); ctx.lastVerb = "block"; ctx.lastActor = who; ctx.note(who); return true
        }
        // "everyone has creatures" / "each player has a 2/2": one such creature for every player at the table.
        Regex("""^(?:everyone|everybody|each player|every player|all players|we all|all of us) (?:has|have|controls?) (?:an? |one |\d+ |two |three )?(?:(\d+/\d+)s? )?(creatures?|tokens?|\d+/\d+s?)$""").find(c)?.let { r ->
            val pt = r.groupValues[1].ifEmpty { Regex("""^\d+/\d+""").find(r.groupValues[2])?.value ?: "" }
            for (id in ctx.playerIds()) { describedCreatures("a ", pt, if (r.groupValues[2].startsWith("token")) "token" else "creature", id, ctx, ""); ctx.note(id) }
            return true
        }
        // "each of my opponents draws a card" / "my opponents each draw two cards": a draw for every opponent.
        Regex("""^(?:each of )?(?:my |the )?opponents (?:each )?(?:draws?|drew) (a|an|one|\d+|two|three|four) cards?$|^each opponent (?:draws?|drew) (a|an|one|\d+|two|three|four) cards?$""").find(c)?.let { r ->
            val n = number(r.groupValues[1].ifEmpty { r.groupValues[2] }) ?: 1
            for (id in ctx.playerIds().filter { it != "me" }) ctx.events += EventSpec("draw", player = id, amount = n)
            return true
        }
        ...
        // "whose creature is it now?" / "who controls it?": the controller once everything has resolved.
        Regex("""^(?:whose (?:creature|permanent|card|c\d+) is (?:it|that|(?:the |my |their )?(c\d+))|who controls (?:it|that|(?:the |my |their )?(c\d+))|who (?:owns|gets|has) (?:it|that|(?:the |my |their )?(c\d+)))(?: now| then| after that| after this)?$""").find(c)?.let { r ->
            val ph = r.groupValues.drop(1).firstOrNull { it.isNotEmpty() }
            val id = if (ph != null) m.cards[ph]?.let { objectIdFor(it, ctx) } ?: return@let else ctx.lastMentioned?.takeIf { it in ctx.objects } ?: ctx.events.lastOrNull { it.verb == "cast" || it.verb == "activate" }?.targets?.firstOrNull { it in ctx.objects } ?: return@let
            ctx.asks += EventSpec("ask", obj = id, to = "controller"); return true
        }
        // "Can I Stifle the land drop?": a land play is a special action, not an ability or a spell.
        Regex("""^(?:(c\d+)|stifle|counter|respond to|stop) (?:the |their |a |that )?land ?(?:drop|play)$""").find(c)?.let { r ->
            if (r.groupValues[1].isNotEmpty() && m.cards[r.groupValues[1]]?.let { it.isSpellOnly || it.display in setOf("Stifle", "Trickbind", "Disallow", "Voidslime", "Tale's End", "Nimble Obstructionist") } != true) return@let
        ...
        Regex("""^(?:kills?|finish(?:es)? off|burns? out|shoots?) (my opponent|the opponent|them|him|her|it|that|(?:my |the |their )?c\d+|(?:the |their )?\d+/\d+)(?: (?:at|who is at|who's at) (\d+)(?: life)?)?$""").find(c)?.let { r ->
            ...
            val victim = when {
                ...
                w in setOf("it", "that") -> ctx.lastMentioned?.takeIf { it in ctx.objects && ctx.objects.getValue(it).controller != who } ?: ctx.objects.values.lastOrNull { it.controller != who && it.zone == "battlefield" && isCreatureName(it.card.name) }?.id ?: return@let
                else -> targetsIn("targeting $w", m, ctx).firstOrNull() ?: return@let
            }
            if (r.groupValues[2].isNotEmpty() && victim in ctx.playerIds()) { ctx.life[victim] = r.groupValues[2].toInt(); ctx.note(victim) }
            if (ctx.objects.values.none { it.controller == who && it.zone == "battlefield" && it.card.oracleId != null } && ctx.objects.values.none { it.controller == who && it.zone == "hand" }) return@let
            ctx.events += EventSpec("kill", player = who, targets = listOf(victim)); ctx.lastActor = who
            if (victim in ctx.playerIds()) ctx.asks += EventSpec("ask", player = victim, to = "playerDie") else ctx.asks += EventSpec("ask", obj = victim, to = "die")
            return true
        }
        // "cast a sorcery" / "cast my creature spell": a spell known only by its type.
        Regex("""^(?:casts?|plays?) (?:an? |my |the )(sorcery|instant|artifact|enchantment|creature spell|creature)(?: spell)?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            ctx.events += EventSpec("cast", player = who, card = CardRef(name = (if (r.groupValues[1].startsWith("i") || r.groupValues[1].startsWith("a") || r.groupValues[1].startsWith("e")) "an " else "a ") + r.groupValues[1])); ctx.lastActor = who; ctx.lastVerb = "cast"; ctx.note(who); return true
        }
        // "3 white sources" / "and four lands open": mana available, said by colour or as sources.
        Regex("""^(?:and |with |plus |i have |having )?(\d+|two|three|four|five|six|seven|eight) (?:untapped |open )?(?:(?:white|blue|black|red|green|colou?red|mana) )?sources(?: untapped| open| available)?$|^(?:and |with |plus )?(\d+|two|three|four|five|six|seven|eight) (?:(?:white|blue|black|red|green) )?lands? (?:untapped|open|available)$""").find(c)?.let { r ->
            val who = actor ?: subject ?: ctx.lastOwner ?: "me"
            val n = number(r.groupValues[1].ifEmpty { r.groupValues[2] }) ?: return@let
            setMana(who, n, ctx)
        ...
        Regex("""^(?:respond(?:s|ed)? with|answers? with|in response (?:i |we )?(?:activate|use|crack|sacrifice|sac)) (?:my |the |an? )?(c\d+)$""").find(c)?.let { r ->
            val card = m.cards.getValue(r.groupValues[1])
            if (!(card.typeLine.contains("Artifact") || card.typeLine.contains("Enchantment")) || card.typeLine.contains("Creature") || card.typeLine.contains("Instant")) return@let
            val who = actor ?: subject ?: "me"
            val id = objectIdFor(card, ctx) ?: addObject(card, who, false, ctx)
            ctx.events += EventSpec("activate", player = who, obj = id); ctx.lastActor = who; ctx.lastMentioned = id
            ctx.notes += "\"Respond with ${card.display}\" is read as activating the one ${if (who == "me") "you" else "they"} already control: a${if (card.typeLine.startsWith("Artifact") || card.typeLine.startsWith("Enchantment")) "n" else ""} ${card.typeLine.substringBefore(" —").lowercase()} spell can't be cast in response."; return true
        }
        // "2 nonland permanents" / "three lands" left on its own after "I have 5 life and …": the count, for the asker.
        Regex("""^(\d+|two|three|four|five|six|seven|eight) ((?:nonland |other |untapped )?(?:permanents|lands|artifacts|enchantments|planeswalkers|creatures))$""").find(c)?.let { r ->
            val who = actor ?: subject ?: ctx.lastOwner ?: "me"
            return readClause((if (who == "me") "i have " else if (who == "opp") "they have " else "@$who has ") + r.groupValues[1] + " " + r.groupValues[2], m, ctx)
        }
        // "an empty library" / "my library is empty" / "no cards left in my library".
        Regex("""^(?:(?:with |has |have )?an? empty library|(?:my |their |his |her )?library is empty|(?:with |has |have )?no cards? (?:left )?in (?:my |their |his |her |the )?library|(?:my |their )?library has no cards(?: left)?)$""").find(c)?.let {
            val who = possessiveOwner(Regex("""\b(my|their|his|her)\b""").find(c)?.value?.plus(" ") ?: "", ctx, m) ?: actor ?: subject ?: "me"
            ctx.librarySize[who] = 0; ctx.note(who)
            ctx.notes += "${if (who == "me") "Your" else "Their"} library is read as empty (0 cards)."; return true
        }
        // "my commander is a 3/3": the commander, sized; "I attack with my commander": it attacks.
        Regex("""^(?:my |their |his |her )?commander is (?:an? )?(\d+)/(\d+)((?: [a-z]+)*)$""").find(c)?.let { r ->
            val who = if (c.startsWith("their") || c.startsWith("his") || c.startsWith("her")) pronounPlayer(ctx, "their") else actor ?: subject ?: "me"
            val existing = ctx.objects.values.lastOrNull { it.commander && it.controller == who }
            val id = existing?.id ?: describedCreatures("a ", "${r.groupValues[1]}/${r.groupValues[2]}", "creature", who, ctx, r.groupValues[3].trim()).firstOrNull() ?: return@let
            ctx.objects[id] = ctx.objects.getValue(id).copy(commander = true)
            if (existing != null && existing.card.name in setOf("a commander", "a creature")) ctx.objects[id] = ctx.objects.getValue(id).copy(card = CardRef(name = "a ${r.groupValues[1]}/${r.groupValues[2]} creature"))
            ctx.lastMentioned = id; ctx.lastOwner = who; return true
        }
        Regex("""^(?:attacks?|attacking|swings?|swinging) (?:with )?(?:my |their |his |her )?commander(?: (?:into|at) (me|them|him|her|my opponent|the opponent|@\w+))?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            val id = commanderObject(who, ctx)
            val defender = planeswalkerFor(r.groupValues[1], ctx) ?: when (val d = r.groupValues[1]) { "" -> ctx.other(who) ?: "opp"; "me" -> "me"; else -> if (d.startsWith("@")) d.removePrefix("@") else pronounPlayer(ctx, d.substringAfterLast(' ')) }
        ...
        // "Opponent casts Abrupt Decay on my Tarmogoyf. It's a 4/5.": the size said in passing. It is noted; where the
        // answer depends on it (a CDA creature's graveyards weren't given) the note says so.
        Regex("""^(?:it's|it is|that's|that is|he's|she's) (?:an? |only an? |just an? )?(\d+)/(\d+)(?: right now| now| currently| at the moment)?$""").find(c)?.let { r ->
            val id = ctx.lastMentioned?.takeIf { it in ctx.objects } ?: ctx.events.lastOrNull { it.verb == "cast" || it.verb == "activate" }?.targets?.firstOrNull { it in ctx.objects } ?: return@let
            val o = ctx.objects.getValue(id)
            if (!isCreatureName(o.card.name)) return@let
            ctx.notes += "${o.card.name} is said to be ${r.groupValues[1]}/${r.groupValues[2]}; its size is worked out from the card and what was described, and the answer above doesn't turn on it unless it says so."
            return true
        }
        // "my opponent's Tarmogoyf is a 0/1 (is that right?)": a claim about its size, checked by asking for it.
        Regex("""^($possPrefix)?(c\d+) (?:is|would be|should be|is now) (?:an? |only an? |just an? )?(\d+)/(\d+)(?: right now| now| then)?$""").find(c)?.let { r ->
            val card = m.cards[r.groupValues[2]] ?: return@let
            if (!card.typeLine.contains("Creature", true) && !card.typeLine.contains("Vehicle", true)) return@let
        ...
        Regex("""^(?:(?:has|have|holds?|holding|with) )?an? (white|blue|black|red|green) card(?: in (?:my |their |his |her )?hand)?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"; ctx.notes += "A ${r.groupValues[1]} card in ${if (who == "me") "your" else "their"} hand is noted (what an alternative cost like Force of Will's exiles)."; ctx.note(who); return true
        }
        // "I have only lands in hand" / "their hand is all lands": land cards, as many as a stated hand size or one.
        // Thoughtseize then finds nothing to take, instead of asking what the hand holds.
        (Regex("""^(?:(?:has|have|holds?|holding|with) )?(?:only|nothing but|just|all|solely) lands?(?: cards?)? in ($possPrefix)?hand$""").find(c)
            ?: Regex("""^(my |their |his |her )hand is (?:all|only|nothing but|just|entirely) lands$""").find(c))?.let { r ->
            val who = possessiveOwner(r.groupValues[1], ctx, m) ?: actor ?: subject ?: "me"
            val n = ctx.handSize[who] ?: 1
            repeat(n) { var id = slug("a land"); var k = 2; while (ctx.objects.containsKey(id)) id = slug("a land") + "_" + (k++); ctx.objects[id] = ObjectSpec(id, CardRef(name = "a basic land"), controller = who, zone = "hand") }
            if (ctx.handSize[who] == null) ctx.notes += "${if (who == "me") "Your" else "Their"} hand is read as one land card, since only its being all lands was said; say how many cards for a precise count."
            ctx.note(who); return true
        }
        ...
        // "4 lands untapped" on its own, split off "they attack with 3 creatures and 4 lands untapped": the mana of
        // whoever was just acting.
        Regex("""^(?:only |just )?(\d+|one|two|three|four|five|six|seven|eight|nine|ten|no|zero) (?:(?:untapped |open )(?:lands?|mana sources?)|(?:lands?|mana sources?) (?:untapped|open|available|up|left)|mana (?:open|available|up|left|untapped))$""").find(c)?.let { r ->
            val who = actor ?: ctx.lastActor ?: "me"; val n = if (r.groupValues[1] in setOf("no", "zero")) 0 else number(r.groupValues[1]) ?: return@let
            setMana(who, n, ctx); ctx.notes += "${if (who == "me") "You have" else (ctx.players[who] ?: "Your opponent") + " has"} $n mana available; costs are checked against that."; return true
        }
        ...
        Regex("""^(?:only )?(?:has|have|with|got|control|controls)(?: only)? (\d+|two|three|four|five|six|seven|eight|nine|ten) lands?(?: in play| on the battlefield| out)?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"; val n = number(r.groupValues[1]) ?: return@let
            setMana(who, n, ctx); if (actor != null) ctx.lastActor = actor
        ...
        if (Regex("""^(?:with |has |having |it has |at |is at |gets? |gains?ed? |gains? |is given |takes? )?(?:\d+|\w+) (?:loyalty(?: counters?)?|(?:[+-]\d+/[+-]\d+|[a-z]+) counters?(?: on it)?|damage(?: marked)?(?: on it)?)$""").matches(c) || Regex("""^(?:with |has )?(?:an? )?[+-]\d+/[+-]\d+ (?:pump|bonus|boost)(?: from .*)?$""").matches(c)) {
            // "I bolt a 1/4, it has 2 damage": after a cast the last thing mentioned is the spell, and the thing
            // meant is what the spell was aimed at. Returning "unread" here also stopped every later rule trying.
            val id = ctx.lastMentioned?.takeIf { ctx.objects.containsKey(it) }
                ?: ctx.events.lastOrNull { it.verb == "cast" }?.targets?.firstOrNull { it in ctx.objects }
            if (id != null) { applyStateWords(id, "with " + c.replace(Regex("""^(?:with |has |having |it has |at |is at |gets? |gains?ed? |gains? |is given |takes? )"""), ""), ctx); return true }
        }
        ...
        // "my 2/2 has 1 damage", "it took 2 damage this turn": damage already marked, part of the board rather
        // than something happening now. "Last turn" is the exception: damage wears off at cleanup (514.2).
        Regex("""^(?:$possPrefix)?(c\d+|it|that|creature|guy|dude|\d+/\d+)(?: creature)? (?:has|have|took|takes|was dealt|were dealt|got|gets|is dealt) (\d+) damage(?: marked)?(?: on it)?( already| this turn| earlier| earlier this turn| last turn| on their turn| on my turn| last combat)?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: ctx.lastOwner ?: "me"
            val w = r.groupValues[1]
            val id = when {
                // "I bolt a creature that already has 2 damage": "it" is the creature the spell was just aimed at.
                w == "it" || w == "that" -> ctx.lastMentioned?.takeIf { it in ctx.objects }
                    ?: ctx.events.lastOrNull { it.verb == "cast" }?.targets?.firstOrNull { it in ctx.objects }
                    ?: ctx.objects.values.lastOrNull { it.zone == "battlefield" && isCreatureName(it.card.name) }?.id ?: return@let
                w == "creature" || w == "guy" || w == "dude" -> ctx.objects.values.lastOrNull { it.controller == who && it.zone == "battlefield" && isCreatureName(it.card.name) }?.id ?: describedCreatures("a ", "", "creature", who, ctx).firstOrNull() ?: return@let
                Regex("""^\d+/\d+$""").matches(w) -> ctx.objects.values.lastOrNull { it.controller == who && it.zone == "battlefield" && (it.card.name ?: "").startsWith("a $w") }?.id ?: describedCreatures("a ", w, "creature", who, ctx).firstOrNull() ?: return@let
                else -> m.cards[w]?.let { card -> objectIdFor(card, ctx) ?: addObject(card, who, false, ctx) } ?: return@let
            }
        ...
        // "my Blood Artist & their Grizzly Bears both die": two named permanents leave at the same time, so each
        // sees the other go (603.10a). Read one after the other, the first to die missed the second.
        Regex("""^((?:my |their |his |her |the )?)(c\d+) & ((?:my |their |his |her |the )?)(c\d+) both (?:die|died|dies|are destroyed|get destroyed)$""").find(c)?.let { r ->
            fun owner(p: String) = when (p.trim()) { "my" -> "me"; "their", "his", "her" -> pronounPlayer(ctx, "their"); else -> ctx.lastOwner ?: "me" }
            val a = m.cards.getValue(r.groupValues[2]).let { card -> objectIdFor(card, ctx) ?: addObject(card, owner(r.groupValues[1]), false, ctx) }
            val b = m.cards.getValue(r.groupValues[4]).let { card -> objectIdFor(card, ctx) ?: addObject(card, owner(r.groupValues[3]), false, ctx) }
            if (a == b) return@let
            ctx.events += EventSpec("leave", obj = a, to = "graveyard", targets = listOf(b))
            ctx.lastMentioned = b; return true
        }
        ...
        Regex("""^(?:kills?|killed|destroys?|destroyed|exiles?|exiled|bounces?|bounced|removes?|removed|nukes?|nuked) (?:it|that|them|him|her)$""").find(c)?.let {
            ...
            val killer = actor ?: ctx.lastActor ?: "me"
            // "I cast Clone copying X, then I bounce it": "it" is the permanent just cast, once it has resolved.
            castPermanentObject(ctx)?.let { id ->
            ...
            val named = ctx.lastMentioned?.takeIf { it in ctx.objects && ctx.objects.getValue(it).zone == "battlefield" }
            // "I have Walking Ballista … can I kill it": nobody kills their own last-named permanent; "it" is the other side's creature.
            val id = named?.takeIf { ctx.objects.getValue(it).controller != killer || ctx.objects.values.none { o -> o.zone == "battlefield" && o.controller != killer && isCreatureName(o.card.name) } }
                ?: ctx.objects.values.lastOrNull { it.zone == "battlefield" && it.controller != killer && isCreatureName(it.card.name) }?.id
        ...
        Regex("""^(?:an? |the |my |their |another |also |(\d+|two|three|four|five) )?(?:(tapped|untapped) )?(c\d+)(?:'s)?(?: (?:is |are )?(?:tapped|untapped))?(?: out| in play| on the battlefield| on board| on the field)?((?: (?:i|they|he|she|we) (?:just )?(?:played|cast|dropped|resolved)(?: this turn| earlier this turn| earlier| last turn| a turn ago)?)?)$""").find(c)?.let { r ->
            ...
            // "They block with a 2/2. I Giant Growth after damage.": an instant named by the other player after a
            // block or attack is them casting it, not one more blocker or attacker.
            if (card.isSpellOnly && actor != null && ctx.lastVerb in setOf("block", "attack") && count == 1) {
                emitCast(actor, card, "", m, ctx); return true
            }
            when (ctx.lastVerb) {
                "have" -> {
                ...
                // nothing before it to continue, is one that player controls. Read as nothing it wasn't there.
                    ...
                    val who = actor ?: subject ?: ctx.lastOwner ?: "me"
                    // "Two Blood Artists. I Wrath.": that many of them, each its own permanent.
                    val id = if (count > 1) { var last = ""; repeat(count) { last = addObject(card, who, isTapped, ctx, allowDuplicate = true) }; last }
                             else objectIdFor(card, ctx) ?: addObject(card, who, isTapped, ctx)
                    ctx.lastMentioned = id; ctx.lastOwner = who; ctx.note(who); return true
                }
            }
        }
    ...
    private fun readClauseB(clause00: String, c0: String, actor: String?, subject: String?, m: Marked, ctx: Ctx): Boolean {
        ...
        // "they hit me with two 2/2 infect creatures": an attack by that many described creatures, unblocked.
        Regex("""^(?:hits?|hitting|swings? at|swinging at|attacks?|connects? with) (me|them|him|her|my opponent|the opponent|@\w+) with (two|three|four|five|\d+) (\d+/\d+)s?((?: (?!creatures?\b)[a-z]+)*)(?: creatures?| tokens?)?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: ctx.lastActor ?: "opp"
            val defender = planeswalkerFor(r.groupValues[1], ctx) ?: when (val d = r.groupValues[1]) { "me" -> "me"; else -> if (d.startsWith("@")) d.removePrefix("@").also { ctx.players.putIfAbsent(it, m.players[it] ?: it) } else pronounPlayer(ctx, d.substringAfterLast(' ')) }
            val ids = describedCreatures(r.groupValues[2] + " ", r.groupValues[3], "creature", who, ctx, r.groupValues[4].trim())
            if (ids.isEmpty()) return@let
            for (id in ids) ctx.events += EventSpec("attack", player = who, obj = id, targets = listOf(defender))
            ctx.lastActor = who; ctx.lastVerb = "attack"; ctx.lastMentioned = ids.last(); ctx.note(defender); return true
        }
        // "I hit my opponent" with the creature said earlier ("my 2/2 with a Sword"): it attacks and connects.
        Regex("""^(?:hits?|hitting|connects? with|smacks?) (me|them|him|her|my opponent|the opponent|@\w+)(?: with it)?(?: unblocked)?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: ctx.lastActor ?: "me"
            val defender = planeswalkerFor(r.groupValues[1], ctx) ?: when (val d = r.groupValues[1]) { "me" -> "me"; else -> if (d.startsWith("@")) d.removePrefix("@").also { ctx.players.putIfAbsent(it, m.players[it] ?: it) } else pronounPlayer(ctx, d.substringAfterLast(' ')) }
            val id = ctx.lastMentioned?.takeIf { it in ctx.objects && ctx.objects.getValue(it).controller == who && isCreatureName(ctx.objects.getValue(it).card.name) }
        ...
        Regex("""^(?:eats?|munch(?:es)?|exiles?|snacks? on) (it|that|the card|that card|(?:the |their |my )?c\d+)(?: from (?:the |their |my )?graveyard)?(?: with (?:my |the )?(c\d+))?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            val eater = r.groupValues[2].takeIf { it.isNotEmpty() }?.let { m.cards[it] }?.let { objectIdFor(it, ctx) }
                ?: ctx.objects.values.firstOrNull { it.controller == who && it.zone == "battlefield" && it.card.name in setOf("Scavenging Ooze", "Deathrite Shaman", "Withered Wretch", "Cemetery Reaper", "Loaming Shaman") }?.id ?: return@let
            val w = r.groupValues[1]
            val id = if (w in setOf("it", "that", "the card", "that card")) ctx.lastMentioned?.takeIf { it in ctx.objects } ?: return@let
                     else m.cards[Regex("""c\d+""").find(w)!!.value]?.let { objectIdFor(it, ctx) } ?: return@let
            ctx.events += EventSpec("activate", player = who, obj = eater, targets = listOf(id)); ctx.lastActor = who; return true
        }
        Regex("""^counters? (?:it|that|that spell|the spell)(?: with (?:an? |the |my |their )?(c\d+))?(?: for free| for its alternative cost| by (?:exiling|pitching) a blue card| pitching a blue card| exiling a blue card| with (?:its )?alt(?:ernative)? cost)?$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            ...
            val counter = r.groupValues[1].takeIf { it.isNotEmpty() }?.let { m.cards.getValue(it) }
                ...
                // "My opponent has Mana Drain. They counter it.": the card they were said to hold is what they counter with.
                ?: ctx.objects.values.lastOrNull { o -> o.zone == "hand" && o.controller == who && names.lookup(Names.normalize(o.card.name ?: ""))?.let { needsSpellTarget(it) } == true }
                    ?.let { o -> names.lookup(Names.normalize(o.card.name!!)) }?.also { ctx.notes += "\"Counter it\" is read as casting ${it.display} from ${if (who == "me") "your" else "their"} hand." }
            val free = if (Regex("""\b(?:for free|alt(?:ernative)? cost|pitching|exiling)\b""").containsMatchIn(c)) " for free" else ""
            if (counter != null) { ctx.inHand[who]?.remove(counter); emitCast(who, counter, " targeting " + slug(targetName) + ":spell" + free, m, ctx) }
        ...
        Regex("""^(?:have|has|got|control|controls|controlling|'ve got|am playing|is playing|are playing|run|running)\s+(?:an? |the |my |their |(\d+|two|three|four|five)(?: copies of| copys of)? )?(?:(commander )|my commander )?((?:(?:hexproof|indestructible|flying|trample|lifelink|deathtouch|haste|vigilance|reach|menace|shroud|unblockable|tapped|untapped) )*)(c\d+)(?:'s)?(.*)$""").find(c)?.let { r00 ->
            ...
            Regex("""^ (?:on|attached to|equipped to|enchanting|equipping) (my opponent's|the opponent's|their|his|her|my|our|@\w+'s|an? ) ?(?:(\d+/\d+) )?($creatureKinds)$""").find(rest)?.let { a ->
                val hostOwner = when (val w = a.groupValues[1].trim()) { "my", "our", "a", "an" -> owner; else -> if (w.startsWith("@")) w.removePrefix("@").removeSuffix("'s") else pronounPlayer(ctx, "their") }
                val host = describedCreatures("a ", a.groupValues[2], a.groupValues[3], hostOwner, ctx, "").firstOrNull() ?: return@let
                ctx.objects[hostId] = ctx.objects.getValue(hostId).copy(attachedTo = host); ctx.lastVerb = "have"; ctx.lastOwner = owner; ctx.lastActor = owner; ctx.lastMentioned = host; return true
            }
        ...
        Regex("""^(?:gets?|getting|fetch(?:es)?|finds?|tutors?(?: for| up)?|grabs?|searches? (?:for|up)) (?:an? |the |my )?(c\d+)$""").find(c)?.let { r ->
            ...
            val card = m.cards.getValue(r.groupValues[1])
            // The spell just cast does the searching: the card is put in the library for it to find, and the
            // engine says whether it fits what the spell looks for ("can I fetch a Breeding Pool with Rampant Growth?").
            val i = ctx.events.indexOfLast { it.verb == "cast" && it.player == who }
            if (i >= 0 && ctx.events[i].to == null && ctx.events[i].card?.name?.let { n -> names.lookup(Names.normalize(n))?.isSpellOnly } == true) {
                val id = addObject(card, who, false, ctx, zone = "library", allowDuplicate = true)
                ctx.events[i] = ctx.events[i].copy(to = "put:$id"); ctx.lastMentioned = id
                ctx.notes += "${card.display} is read as the card ${ctx.events[i].card?.name} is asked to find."; return true
            }
        ...
        Regex("""^equips? (?:my |the )?(c\d+|it) (?:to|onto) (?:my |the |an? )?(c\d+|(?:\d+/\d+ )?(?:[a-z]+ )*?token|(?:\d+/\d+)(?: [a-z]+)*|it|that)$""").find(c)?.let { r ->
            val who = actor ?: subject ?: "me"
            ...
            val hostId = if (r.groupValues[2].startsWith("c") && r.groupValues[2].drop(1).all { it.isDigit() }) m.cards.getValue(r.groupValues[2]).let { objectIdFor(it, ctx) ?: addObject(it, who, false, ctx) }
                         // "I animate Mutavault and equip Batterskull to it": the permanent just spoken of.
                         else if (r.groupValues[2] in setOf("it", "that")) ctx.lastMentioned?.takeIf { it in ctx.objects && it != r.groupValues[1].let { p -> m.cards[p]?.let { c -> objectIdFor(c, ctx) } } }
                             ?: ctx.events.lastOrNull { it.verb == "activate" && it.player == who && it.obj != null }?.obj ?: return@let
        ...
        Regex("""^(?:there(?: is| are|'s) )?(?:(?:i |we |they )?(?:have|has) )?(?:an? |$possPrefix)?(c\d+) (?:is |are )?(?:on|attached to|enchanting|equipping) (?:an? |($possPrefix))?(\d+/\d+|creature|guy|dude|token)((?: [a-z]+)*)$""").find(c)?.let { r ->
            val attCard = m.cards.getValue(r.groupValues[1])
            val owner = possessiveOwner(r.groupValues[2], ctx, m) ?: actor ?: "me"
            val hostId = (if (Regex("""^\d+/\d+$""").matches(r.groupValues[3])) describedCreatures("a ", r.groupValues[3], "creature", owner, ctx, r.groupValues[4].trim()) else describedCreatures("a ", "", if (r.groupValues[3] == "token") "token" else "creature", owner, ctx, r.groupValues[4].trim())).firstOrNull() ?: return@let
            val attId = objectIdFor(attCard, ctx) ?: addObject(attCard, actor ?: "me", false, ctx)
            ctx.objects[attId] = ctx.objects.getValue(attId).copy(attachedTo = hostId); ctx.lastMentioned = hostId; ctx.note(owner); return true
        }
    ...
    private fun readClauseC(clause00: String, c0: String, actor: String?, subject: String?, m: Marked, ctx: Ctx): Boolean {
        ...
        Regex("""^(c\d+)s?\s+((?:for (?:x ?= ?)?\d+ )?(?:(?:targeting|on|at|(?:split|divided) (?:between|among)|between|among)\s+)?(?:the|my|their|that|this|it|them|me|an?|c\d+|\d+/\d+|two|three|four)\b.*)$""").find(c)?.let { r ->
            ...
            if (card.typeLine.contains("Land")) {
                val who = subject ?: ctx.lastActor ?: "me"
                val id = objectIdFor(card, ctx) ?: addObject(card, who, false, ctx)
                val targets = targetsIn("targeting " + r.groupValues[2], m, ctx).let { t ->
                    if (t != listOf(id)) t else listOfNotNull(ctx.events.lastOrNull { it.verb == "attack" }?.obj)
                }
                ctx.events += EventSpec("activate", player = who, obj = id, targets = targets); ctx.lastActor = who; return true
            }
            val tail = r.groupValues[2]
            emitCast(subject ?: ctx.lastActor ?: "me", card, if (Regex("""^(?:for (?:x ?= ?)?\d+ )?(?:split|divided|between|among|targeting|on|at)\b""").containsMatchIn(tail)) " $tail" else " targeting " + tail, m, ctx); return true
        }
        ...
        Regex("""^(?:$activateVerbs)\s+(?:an? |the |their |my )?(c\d+)(?:'s)?(?: ability)?(.*)$""").find(c)?.let { r ->
            ...
            val levelRe = Regex("""^\s*level[- ]?up ability\b""")
            val levelUp = levelRe.containsMatchIn(r.groupValues[2])
            val restForTargets = r.groupValues[2].replace(levelRe, "").replace(xRe, "").replace(timesRe, "").let { s -> colour?.let { s.replace(colourRe, "") } ?: s }.trim()
            val targets = targetsIn(restForTargets, m, ctx).let { t ->
                if (t != listOf(id)) t
                else listOfNotNull(ctx.events.lastOrNull { it.verb == "attack" }?.obj ?: ctx.objects.values.lastOrNull { it.id != id && it.zone == "battlefield" && isCreatureName(it.card.name) }?.id)
            }.ifEmpty { if (colour != null) listOf(id) else emptyList() }
            repeat(times) { ctx.events += EventSpec("activate", player = who, obj = id, targets = targets, amount = xValue, to = colour?.let { "color:$it" } ?: if (levelUp) "levelup" else null) }
            ctx.lastActor = who; return true
        }
        ...
        // "attack with a 3/3 equipped with Sword of Fire and Ice" / "swing with a 2/2 wearing Rancor": the creature nobody
        // named is described, the Equipment or Aura attached to it, and it attacks.
        Regex("""^(?:attacks?|attacking|swings?|swinging)(?: with)?\s+(an? |my |the |two |three |\d+ )?(?:(\d+/\d+)\s*)?(?:($kwNouns)\s+)?(?:($creatureKinds)\s+)?(?:equipped with|enchanted with|with|wearing|carrying) (?:an? |my |the )?(c\d+)(?: attached| on it| equipped)?(?: (?:at|into) (me|them|him|her|my opponent|the opponent|@\w+))?$""").find(c)?.let { r ->
            if (r.groupValues[2].isEmpty() && r.groupValues[4].isEmpty()) return@let
            val eqCard = m.cards[r.groupValues[5]] ?: return@let
            if (!eqCard.typeLine.contains("Equipment") && !eqCard.typeLine.contains("Aura")) return@let
            val who = actor ?: subject ?: "me"
            val kw = r.groupValues[3].let { if (it.isEmpty()) "" else it.removeSuffix("s").replace("flier", "flying").replace("flyer", "flying").replace("trampler", "trample").replace("deathtoucher", "deathtouch").replace("lifelinker", "lifelink").replace("striker", "strike") }
            val ids = describedCreatures(r.groupValues[1].ifEmpty { "a " }, r.groupValues[2], r.groupValues[4].ifEmpty { "creature" }, who, ctx, kw)
            val host = ids.firstOrNull() ?: return@let
            val eq = objectIdFor(eqCard, ctx) ?: addObject(eqCard, who, false, ctx)
            ctx.objects[eq] = ctx.objects.getValue(eq).copy(attachedTo = host)
            val defender = r.groupValues[6].let { d -> when { d.isEmpty() -> ctx.other(who) ?: "me"; d.startsWith("@") -> d.removePrefix("@").also { ctx.players.putIfAbsent(it, m.players[it] ?: it) }; d == "me" -> "me"; else -> pronounPlayer(ctx, d.substringAfterLast(' ')) } }
            ids.forEach { ctx.events += EventSpec("attack", player = who, obj = it, targets = listOf(defender)) }
            ctx.lastActor = who; ctx.lastVerb = "attack"; ctx.lastMentioned = host; ctx.note(defender); return true
        }
        Regex("""^(?:attacks?|attacking|swings?|swinging)(?: (@\w+|me|them|him|her|my opponent|the opponent|opponent|(?:$possPrefix)?c\d+|it|that))?(?: with)?\s+(an? |the |my |their |\d+ |two |three |four |five )?(?:(\d+/\d+)s?\s*)?(?:(red|green|white|blue|black|colorless|flying) )?(?:($kwNouns)\b ?)?($creatureKinds)?( tokens?)?(?: with ([a-z ,&]+?))?(?: plus .*)?((?: but .*| and .*)?)$""").find(c)?.let { r0 ->
            ...
            val defender: String = r.groupValues[1].takeIf { it.isNotEmpty() }?.let { d ->
                when {
                    ...
                    (d == "him" || d == "her") && planeswalkerOf() != null -> planeswalkerOf()
        ...
        Regex("""^($possPrefix|an? )?(c\d+|it|that|\d+/\d+)(?: creature)?(?: with ($kwPhrase(?:(?:,|,? and|,? &) $kwPhrase)*))? (?:fights?|fought) ($possPrefix|an? )?(c\d+|\d+/\d+)(?: creature)?(?: with ($kwPhrase(?:(?:,|,? and|,? &) $kwPhrase)*))?$""").find(c)?.let { r0 ->
            // "a 2/2 with deathtouch fights a 2/2 with lifelink": the keywords describe each side.
            val kwA = r0.groupValues[3]; val kwB = r0.groupValues[6]
            val r = object { val groupValues = listOf(r0.groupValues[0], r0.groupValues[1], r0.groupValues[2], r0.groupValues[4], r0.groupValues[5]) }
            ...
            val aId = if (r.groupValues[2] in setOf("it", "that")) ctx.lastMentioned?.takeIf { it in ctx.objects } ?: return@let
                      else if (Regex("""^\d+/\d+$""").matches(r.groupValues[2])) describedCreatures("a ", r.groupValues[2], "creature", whoA, ctx, kwA).firstOrNull() ?: return@let
            ...
            val bId = if (Regex("""^\d+/\d+$""").matches(r.groupValues[4])) describedCreatures("a ", r.groupValues[4], "creature", whoB, ctx, kwB).firstOrNull() ?: return@let
        ...
        Regex("""^($possPrefix)?(c\d+|it|that|\d+/\d+)(?: creature)?(?: with ($kwPhrase(?:(?:,|,? and|,? &) $kwPhrase)*))? (?:is|are|was|were|gets?|got) (?:chump[- ]?|double[- ]?|triple[- ]?)?blocked by (.+)$""").find(c)?.let { r ->
            ...
            val blockers = r.groupValues[4].split(" plus ", " & ").mapNotNull { part0 ->
        ...
        Regex("""^(?:chump[- ]?)?blocks? ($possPrefix|an? )(?:($kwNouns) )?(c\d+|\d+/\d+|creature|guy|dude|$kwNouns)(?: ($kwNouns))?(?: creature)? with (.+)$""").find(c)?.let { r00 ->
            // "I block a first striker with a 2/2": the keyword noun is the whole of the attacker's description.
            val r0 = object { val groupValues: List<String> = if (Regex("""^$kwNouns$""").matches(r00.groupValues[3]) && !cardRef.matches(r00.groupValues[3])) listOf(r00.groupValues[0], r00.groupValues[1], r00.groupValues[3], "creature", r00.groupValues[4], r00.groupValues[5]) else r00.groupValues }
            ...
            val id = ctx.lastMentioned?.takeIf { it in ctx.objects && (isCreatureName(ctx.objects.getValue(it).card.name) || it == stolen) } ?: stolen
                ...
                ?: run {
                    if (ctx.objects.values.none { it.controller == who && it.zone == "battlefield" && isCreatureName(it.card.name) }) {
                        val stand = describedCreatures("a ", "", "creature", who, ctx).firstOrNull() ?: return@let
                        ctx.notes += "No creature of ${if (who == "me") "yours" else "theirs"} was described; a creature stands in as the attacker."
                        ctx.events += EventSpec("attack", player = who, obj = stand, targets = listOf(defender)); ctx.lastActor = who; ctx.lastVerb = "attack"; ctx.lastMentioned = stand; ctx.note(defender); return true
                    }
                    if (ctx.events.lastOrNull()?.verb in setOf("cast", "activate", "trigger")) ctx.events += EventSpec("resolveAll")
                    ctx.events += EventSpec("attackAll", player = who, targets = listOf(defender)); ctx.lastActor = who; ctx.lastVerb = "attack"; ctx.note(defender); return true
                }
            if (ctx.events.lastOrNull()?.verb in setOf("cast", "activate", "trigger")) ctx.events += EventSpec("resolveAll")
    ...
    private fun askQuestion(clause0: String, m: Marked, ctx: Ctx): Boolean {
        ...
        Regex("""^what (?:are|is|'s) (?:my |their |the |@\w+'s )?(c\d+|it|they)(?: now| then| after that| after this| at that point)?$""").find(clause0)?.let { q ->
            val id = if (q.groupValues[1] == "it" || q.groupValues[1] == "they") ctx.lastMentioned?.takeIf { it in ctx.objects && isCreatureName(ctx.objects.getValue(it).card.name) } ?: return@let else m.cards[q.groupValues[1]]?.takeIf { it.typeLine.contains("Creature") || (!it.isSpellOnly && objectIdFor(it, ctx) != null) }?.let { objectIdForOrCast(it, ctx) ?: addObject(it, "me", false, ctx) } ?: return@let
            ctx.asks += EventSpec("ask", obj = id, to = "pt"); ctx.notes += "\"${restore(clause0, m)}?\" is answered by the outcome below."; return true
        }
        ...
        Regex("""^(?:does|do|did|will|would|is|are) (my|their|his|her|the|@\w+'s) (?:(\d+/\d+)(?: creature)?|creature|token|guy|attacker|blocker|dude|beater) (?:still |going to |gonna )?(trigger|triggers|go off|survive|survives|die|dies|dead|still alive|live|lives|make it)\b""").find(clause0)?.let { q0 ->
            ...
            // "does the attacker die?" / "does my blocker survive?": the creature in that role, not the last one described.
            val role = when { Regex("""\battacker\b""").containsMatchIn(clause0) -> ctx.events.lastOrNull { it.verb == "attack" && it.obj != null }?.obj; Regex("""\bblocker\b""").containsMatchIn(clause0) -> ctx.events.lastOrNull { it.verb == "block" && it.obj != null }?.obj; else -> null }?.let { ctx.objects[it] }
            val id = role ?: (if (pt.isNotEmpty()) ctx.objects.values.lastOrNull { (who == null || it.controller == who) && (it.card.name ?: "").startsWith("a $pt") } else null)
        ...
        // "Can I stop it?" / "can I do anything about it?": whatever the asker controls or holds that answers the spell.
        Regex("""^can (?:i|we) (?:stop|prevent|answer|deal with|do anything about|respond to) (?:it|that|this|the spell|their spell)$""").find(clause0)?.let {
            if (ctx.events.none { it.verb == "cast" && it.player != "me" }) return@let
            ctx.events += EventSpec("stop", player = "me"); ctx.notes += "\"${restore(clause0, m)}?\" is answered by the outcome below."; return true
        }
        ...
        // "does the attacker die?" / "does their attacker take damage?" / "does my blocker survive?": the creature in that role.
        Regex("""^(?:does|do|did|will|would|is|are) (the|their|my|his|her) (attacker|attacking creature|blocker|blocking creature|chump blocker|wall|creature|guy|dude|token|\d+/\d+) (?:still |even )?(die|dies|survive|survives|dead|take (?:any |combat )?damage|takes (?:any |combat )?damage|get damaged|dealt damage|damaged)$""").find(clause0)?.let { r ->
            val role = r.groupValues[2]
            val owner = when (r.groupValues[1]) { "my" -> "me"; "their", "his", "her" -> pronounPlayer(ctx, "their"); else -> null }
            val id = when {
                role.startsWith("attack") -> ctx.events.lastOrNull { it.verb == "attack" && it.obj != null }?.obj
                role.startsWith("block") || role == "chump blocker" -> ctx.events.lastOrNull { it.verb == "block" && it.obj != null }?.obj
                // "does my wall die?": the described creature of that kind.
                else -> ctx.objects.values.lastOrNull { o -> (owner == null || o.controller == owner) && o.zone == "battlefield" && isCreatureName(o.card.name) && (role in setOf("creature", "guy", "dude") || (o.card.name ?: "").contains(role)) }?.id
            }
            id ?: return@let
        ...
        Regex("""^(?:do|does|will|would) (i|we|they) (?:still |even |get to )?untap (?:my|our|their|all my|all of my) lands$""").find(clause0)?.let { r ->
            val who = if (r.groupValues[1] in setOf("i", "we")) "me" else pronounPlayer(ctx, "their")
            ctx.asks += EventSpec("ask", player = who, to = "untapLands"); ctx.note(who)
            ctx.notes += "\"${restore(clause0, m)}?\" is answered by the outcome below."; return true
        }
        ...
        Regex("""^castlater-question (it|that|c\d+)$""").find(clause0)?.let { r ->
            val lastCast = ctx.events.lastOrNull { it.verb == "cast" }
            val id = (if (cardRef.matches(r.groupValues[1])) m.cards[r.groupValues[1]]?.let { objectIdFor(it, ctx) } else lastCast?.targets?.firstOrNull { it in ctx.objects } ?: ctx.lastMentioned?.takeIf { it in ctx.objects }) ?: return@let
            val o = ctx.objects.getValue(id)
            val name = o.card.name ?: id
            val grantor = (lastCast?.card?.name ?: lastCast?.obj?.let { ctx.objects[it]?.card?.name })
            if (o.zone == "graveyard" && grantor in setOf("Snapcaster Mage", "Torrential Gearhulk"))
                ctx.asks += EventSpec("ask", to = "text:No. $grantor gives $name flashback only until end of turn (611.2a), so next turn $name is an ordinary card in your graveyard with no way to cast it. To use it, cast it this turn with flashback, paying its mana cost (702.34a); it is exiled as it resolves.")
            else if (o.zone == "graveyard")
                ctx.asks += EventSpec("ask", to = "text:Only if something lets you: a card in your graveyard can't be cast from there unless it has flashback or another effect allows it at that time (601.3, 702.34a).")
            else return@let
            ctx.notes += "\"can you cast $name next turn?\" is answered by the outcome below."; return true
        }
        // "what happens to my creature?" after its Equipment or Aura is dealt with: the creature's size afterwards.
        Regex("""^what happens to (my |their |his |her |the )?(creature|guy|dude|it|that|c\d+)(?: then| now| after that)?$""").find(clause0)?.let { r ->
            val who = when (r.groupValues[1]) { "my " -> "me"; "their ", "his ", "her " -> pronounPlayer(ctx, "their"); else -> null }
            val id = if (cardRef.matches(r.groupValues[2])) m.cards[r.groupValues[2]]?.let { objectIdFor(it, ctx) } ?: return@let
                     else if (r.groupValues[2] in setOf("it", "that")) ctx.lastMentioned?.takeIf { it in ctx.objects } ?: return@let
                     else ctx.objects.values.lastOrNull { o -> (who == null || o.controller == who) && o.zone == "battlefield" && isCreatureName(o.card.name) && ctx.objects.values.any { a -> a.attachedTo == o.id } }?.id
                         ?: ctx.objects.values.lastOrNull { o -> (who == null || o.controller == who) && o.zone == "battlefield" && isCreatureName(o.card.name) }?.id ?: return@let
            if (!isCreatureName(ctx.objects.getValue(id).card.name)) { ctx.notes += "\"${restore(clause0, m)}?\" is answered by the outcome below."; return true }
            ctx.asks += EventSpec("ask", obj = id, to = "pt"); ctx.notes += "\"${restore(clause0, m)}?\" is answered by the outcome below."; return true
        }
        // "does the trigger still resolve?" / "does the ability still happen?": an ability on the stack outlives its source.
        Regex("""^(?:does|do|will|would) (?:the|its|that|this|their|his|her|my) (?:trigger|triggered ability|ability|etb|etb trigger|death trigger) (?:still |even )?(?:resolve|resolves|happen|happens|go off|work|fire|fires|trigger|triggers)$""").find(clause0)?.let { _ ->
            val id = ctx.events.lastOrNull { it.verb == "trigger" && it.obj != null }?.obj ?: ctx.lastMentioned?.takeIf { it in ctx.objects } ?: return@let
            ctx.asks += EventSpec("ask", obj = id, to = "stillResolves")
            ctx.notes += "\"${restore(clause0, m)}?\" is answered by the outcome below."; return true
        }
        // "does it survive?" / "will it die?": the last-mentioned permanent.
        Regex("""^(?:does|do|did|will|would|is|are) (?:(?:its|the|his|her|their) (?:ability|abilities|trigger|triggered ability) )?(it|that|this|he|she|they|theirs|mine|yours|the other one|the other)?(?:\s*(?:still|even|going to|gonna)\s+)?\s*(trigger|triggers|go off|survive|survives|die|dies|dead|still alive|live|lives|make it)\b""").find(clause0)?.let { q0 ->
            val q = object { val groupValues = listOf(q0.groupValues[0], q0.groupValues[2]) }
            ...
            val last = ctx.events.lastOrNull { it.verb == "cast" }?.takeIf { e -> e.player == "me" && e.card?.name?.let { n -> names.lookup(Names.normalize(n))?.isSpellOnly } == true && ctx.events.indexOf(e) >= ctx.events.indexOfLast { x -> x.verb !in setOf("resolve", "resolveAll", "combatDamage", "damage", "pass") } }?.let { e -> e.targets.firstOrNull { it in ctx.objects } ?: ctx.events.lastOrNull { a -> a.verb == "attack" && a.player == "me" }?.obj
                ...
                ?: ctx.events.lastOrNull { it.verb == "cast" && it.card?.name != null && !it.card.name.startsWith("a ") }?.card?.name
    ...
    private fun emitCast(who: String, card: NameIndex.Entry, restIn: String, m: Marked, ctx: Ctx) {
        ...
        val rest00000 = Regex("""\s*\bwith (?:only |just )?(\d+|one|two|three|four|five|six) (?:untapped |open )?(?:c\d+s?|lands?|mana(?: sources?)?)(?: untapped| open| available| up| left)?$""").find(rest0000r)?.let { r ->
            ...
            setMana(who, n, ctx); ctx.notes += "${if (who == "me") "You have" else (ctx.players[who] ?: "Your opponent") + " has"} $n mana available; costs are checked against that."; rest0000r.removeRange(r.range)
        } ?: rest0000r
        // "they cast Ponder with no mana open": nothing left over once it is cast, so no optional payment is made.
        val rest000000 = Regex("""\s*\bwith (?:no|zero|0|nothing) (?:mana|lands?)(?: open| up| left| available| untapped| floating)?(?: (?:after|afterwards|left over))?$""").find(rest00000)?.let { r ->
            ctx.events += EventSpec("pay", player = who, to = "no")
            ctx.notes += "${if (who == "me") "You cast" else (ctx.players[who] ?: "Your opponent") + " casts"} ${card.display} with no mana left over, so nothing optional (a tax, a \"unless\" cost) can be paid for it."
            rest00000.removeRange(r.range)
        } ?: rest00000
        ...
        val aboutLife = Regex("""\s*\b(me|them|him|her|my opponent|the opponent|@\w+)\s*,?\s+(?:who(?:'s| is| was)?|that(?:'s| is| was)?|is|was)\s+(?:sitting |currently |already )?at (\d+) life\b,?""").find(rest000000)
        ...
        val rest000 = if (aboutLife != null) {
            ...
            rest000000.replaceRange(aboutLife.range, " " + aboutLife.groupValues[1])   // the player is still the target
        ...
        // "Vandalblast not overloaded on my Sol Ring": the negation is read, and the words dropped so they aren't a target.
        val rest = (secondTime?.let { rest1.removeRange(it.range) } ?: rest1).replace(Regex("""\s*\b(?:not overloaded|without overload|unoverloaded|not for (?:its|the) overload cost)\b"""), "")
        ...
        } ?: Regex("""\b(?:choosing|picking|with|taking) (?:both)(?: (?:of the )?modes| of them| the modes)?\b""").find(rest)?.let { listOf(1, 2) }
          // "choosing everything" / "all modes" (Farewell): every mode the card has; the judge clips the list to its count.
          ?: Regex("""\b(?:choosing|picking|with|taking) (?:all|everything|every mode|all of it|all the modes|all modes|all of them|all four|all three)(?: (?:of the )?modes| of them| the modes)?\b""").find(rest)?.let { listOf(1, 2, 3, 4, 5, 6) } ?: emptyList()
        val overload = Regex("""(?<!\bnot )(?<!\bwithout )(?<!\bun)\b(?:overloaded|with overload|for (?:its|the) overload cost|via overload)\b""").containsMatchIn(rest)
        ...
        val xValue = Regex("""\b(?:with|for|where|at) x ?(?:=|equal to|equals|being|of|as) ?(\d+)\b|\bx ?= ?(\d+)\b""").find(rest)?.let { r -> (r.groupValues[1].ifEmpty { r.groupValues[2] }).toIntOrNull() }
            ?: Regex("""\bfor (\d+)(?=\s*$|\s+(?:targeting|at|on|against|split|divided|between|among|in|with|while|when)\b)""").find(rest.trim())?.groupValues?.get(1)?.toIntOrNull()
            ?: payLife
        val kicked = Regex("""\b(?:kicked|with (?:the )?kicker|with kicker paid|paying (?:the )?kicker|kicking it)\b""").containsMatchIn(rest)
        // "for free" / "pitching a blue card" / "for its alternative cost": Force of Will's kind of cost (118.9).
        val altCost = Regex("""\b(?:for free|for its alt(?:ernative)? cost|with (?:its )?alt(?:ernative)? cost|via alt(?:ernative)? cost|(?:by )?(?:pitching|exiling) an? (?:blue|green|black|red|white) card|pitching a card|without paying its mana cost)\b""").containsMatchIn(rest)
        ...
        val modeWord = (Regex("""\b(?:choosing|picking|selecting) (?:the )?((?:$modeVerbs)[a-z0-9' ]*?) (?:and|&) (?:the )?((?:$modeVerbs)[a-z0-9' ]*?)(?:\s+(?:targeting|aimed at|on|at)\b.*)?\s*$""").find(rest.trim())?.let { mm -> mm.groupValues[1].trim() + "|" + mm.groupValues[2].trim() }
            ...
            ?: Regex("""\b(?:choosing|picking|selecting) (?:the )?([a-z][a-z0-9' ]*?)(?: (?:and|&) (?:the )?([a-z][a-z0-9' ]*?))?(?:\s+(?:targeting|aimed at)\b.*)?\s*$""").find(rest.trim())?.takeIf { mm -> mm.groupValues[1].split(" ")[0] !in setOf("both", "all", "neither", "none", "not", "mode", "modes", "to", "it", "them", "that", "this", "one", "two", "three") }?.let { mm -> listOf(mm.groupValues[1], mm.groupValues[2]).filter { it.isNotBlank() }.joinToString("|") { w ->
                // "choosing Shatter and discard": a card name standing for a mode ("Shatter" = destroy the artifact).
                w.trim().replace(Regex("""\bc\d+\b""")) { m.cards[it.value]?.display?.lowercase() ?: it.value } } })?.takeIf { it !in setOf("first", "second", "third", "fourth", "same", "other") }
        ctx.castingCounter = needsSpellTarget(card); ctx.castingCounterName = card.display
        // "choosing counter and bounce on my Bolt and my Bears": the modes are said, then the targets; the modes phrase
        // has to come off before the targets are read, or "on …" was never seen.
        val restForTargets = if (modeWord != null) rest.replace(Regex("""\b(?:choosing|picking|selecting)\b.*?(?=\s+(?:targeting|aimed at|on|at)\s|$)"""), "").trim() else rest
     ...
     */
    /** The object for a card, or — when the card is the permanent spell just cast — the object that cast becomes. */
    private fun objectIdForOrCast(card: NameIndex.Entry, ctx: Ctx): String? =
        objectIdFor(card, ctx) ?: (if (ctx.lastMentioned == "cast:" + slug(card.display)) castPermanentObject(ctx) else null)
    ...
    private fun targetsIn(rest: String, m: Marked, ctx: Ctx): List<String> {
        ...
        Regex("""^(an? |their |my |the )?(\d+)[- ]mana (creature|permanent|artifact|enchantment)$""").find(seg)?.let { r ->
            ...
            val name = "a ${r.groupValues[2]} mana ${r.groupValues[3]}"
            var id = slug(name); var k = 2; while (id in ctx.objects) id = slug(name) + "_" + (k++)
            ctx.objects[id] = ObjectSpec(id, CardRef(name = name), controller = owner)
            ctx.notes += "\"${r.groupValues[2]} mana ${r.groupValues[3]}\" is read as ${if (r.groupValues[3].first() in "aeiou") "an" else "a"} ${r.groupValues[3]} with mana value ${r.groupValues[2]} under ${if (owner == "me") "your" else "your opponent's"} control."
            out += id; return out
        }
        // "blocks the one attacking her": the attacker aimed at that player.
        Regex("""^(?:the )?(?:one|creature|attacker|guy|dude) (?:that is |that's |who is )?attacking (me|us|her|him|them|@\w+|my opponent)$""").find(seg)?.let { r ->
            val who = when (val w = r.groupValues[1]) { "me", "us" -> "me"; "my opponent" -> pronounPlayer(ctx, "opponent"); "her", "him", "them" -> ctx.lastActor ?: return@let; else -> w.removePrefix("@") }
        ...
        Regex("""^(my|their|his|her|my opponent's|the opponent's|@\w+'s) commander$""").find(seg)?.let { r ->
            val who = when (val w = r.groupValues[1]) { "my" -> "me"; "their", "his", "her", "my opponent's", "the opponent's" -> pronounPlayer(ctx, "their"); else -> w.removePrefix("@").removeSuffix("'s") }
            out += commanderObject(who, ctx); return out
        }
        // "Bolt the blocker" / "exile the attacker": the creature in that role in the combat described.
        Regex("""^(?:the |their |my |his |her )?(blocker|blocking creature|chump blocker|chump|attacker|attacking creature)$""").find(seg)?.let { r ->
            val id = if (r.groupValues[1].startsWith("attack")) ctx.events.lastOrNull { it.verb == "attack" }?.obj else ctx.events.lastOrNull { it.verb == "block" }?.obj
            if (id != null) { out += id; return out }
        }
        ...
        // "on my two Grizzly Bears": that many of the named card, each a target (Ashes to Ashes).
        Regex("""^(?:(my|their|his|her|my opponent's|the opponent's|opponent's|the) )?(two|three|four|\d+) (c\d+)s?$""").find(seg)?.let { r ->
            val card = m.cards[r.groupValues[3]] ?: return@let
            val n = number(r.groupValues[2]) ?: return@let
            val owner = when (r.groupValues[1]) { "my" -> "me"; "" , "the" -> ctx.other(ctx.clauseActor ?: ctx.lastActor) ?: "opp"; else -> pronounPlayer(ctx, "their") }
            val have = ctx.objects.values.filter { it.card.oracleId == card.oracleId && it.controller == owner && it.zone == "battlefield" }.map { it.id }.toMutableList()
            while (have.size < n) have += addObject(card, owner, false, ctx, allowDuplicate = true)
            return have.take(n)
        }
        // "their two 2/2s": that many described creatures, each a target.
        Regex("""^(?:(my|their|his|her|my opponent's|the opponent's|opponent's|the) )?(two|three|four|\d+) (\d+/\d+)s?((?: (?!with\b)[a-z]+)*)$""").find(seg)?.let { r ->
            val owner = when (r.groupValues[1]) { "my" -> "me"; "" , "the" -> pronounPlayer(ctx, "their"); else -> pronounPlayer(ctx, "their") }
            val n = number(r.groupValues[2]) ?: return@let
            if (n < 2 || n > 6) return@let
            val ids = describedCreatures(r.groupValues[2] + " ", r.groupValues[3], r.groupValues[4].trim().ifEmpty { "creature" }, owner, ctx, "")
            if (ids.isEmpty()) return@let
            out += ids; return out
        }
        // "their two creatures" / "two tokens": that many creatures with no size given, each a target (Fireball).
        Regex("""^(?:(my|their|his|her|my opponent's|the opponent's|opponent's|the) )?(two|three|four|five|\d+) (creatures|tokens|guys|dudes|blockers|attackers)$""").find(seg)?.let { r ->
        ...
        Regex("""^(?:(my own|my|their|his|her|my opponent's|the opponent's|opponent's|an?|the) )?(?:(attacking|blocking|tapped|untapped) )?(?:(white|blue|black|red|green|colorless) )?(creatures?|lands?|artifacts?|enchantments?|permanents?|planeswalkers?|guys?|dudes?)$""").find(seg)?.let { r0 ->
            // "Condemn on their attacking creature": the state word says what it is doing; an attacking creature
            // nobody said attacked is declared as attacking the speaker.
            val stateWord = r0.groupValues[2]
            val r = object { val groupValues = listOf(r0.groupValues[0], r0.groupValues[1], r0.groupValues[3], r0.groupValues[4]) }
            val head = r.groupValues[1]
            ...
            ctx.objects.values.lastOrNull { o -> o.controller == owner && o.zone == "battlefield" &&
                (if (kind in setOf("creature", "guy", "dude")) isCreatureName(o.card.name) else (o.card.name ?: "").contains(kind, true)) }?.let { o -> out += o.id; return out.also { withState(o.id, stateWord, owner, ctx) } }
            if (kind in setOf("creature", "guy", "dude")) describedCreatures("a ", "", "creature", owner, ctx).firstOrNull()?.let { out += it; ctx.note(owner); return out.also { _ -> withState(it, stateWord, owner, ctx) } }
            ...
            out += id; ctx.note(owner); return out.also { withState(id, stateWord, owner, ctx) }
        }
        // "Doom Blade on my Knight" / "targeting their Elf": a creature named by its type alone is one of the
        // asker's described creatures of that type, or a new one. Read as nothing, the spell went at another creature.
        Regex("""^(?:(my|their|his|her|my opponent's|the opponent's|opponent's|an?|the) )?($creatureKinds)$""").find(seg)?.let { r ->
            val owner = if (r.groupValues[1] == "my") "me" else pronounPlayer(ctx, "their")
            val kind = singularKind[r.groupValues[2]] ?: r.groupValues[2].removeSuffix("s")
            if (kind in setOf("creature", "guy", "dude")) return@let
            ctx.objects.values.lastOrNull { o -> o.controller == owner && o.zone == "battlefield" && Regex("""\b${Regex.escape(kind)}s?\b""", RegexOption.IGNORE_CASE).containsMatchIn(o.card.name ?: "") }?.let { o -> out += o.id; return out }
            val ids = describedCreatures("a ", "", kind, owner, ctx, "")
        ...
        Regex("""(?:the |my |their |an? )?(c\d+)""").find(seg)?.let { t ->
            ...
            val nid = addObject(card, owner, seg.contains("tapped") && !seg.contains("untapped"), ctx)
            ctx.objects[nid] = ctx.objects.getValue(nid).copy(assumed = true); out += nid
        ...
        if (Regex("""^(it|that|that spell|the spell)\b""").containsMatchIn(seg)) {
            ...
            // "I cast Phantasmal Image copying their Titan. Then they Bolt it": the permanent just cast, once it has resolved.
            if (lm?.startsWith("cast:") == true && !spellStillThere) castPermanentObject(ctx)?.let { candidates += it }
    ...
    /** "their attacking creature", "my tapped creature": the state word said of a target that nobody named. */
    private fun withState(id: String, word: String, owner: String, ctx: Ctx) {
        when (word) {
            "attacking" -> if (ctx.events.none { it.verb == "attack" && it.obj == id }) { ctx.events += EventSpec("attack", player = owner, obj = id, targets = listOf(ctx.other(owner) ?: "me")); ctx.note(ctx.other(owner) ?: "me") }
            "tapped" -> ctx.objects[id] = ctx.objects.getValue(id).copy(tapped = true)
            "untapped" -> ctx.objects[id] = ctx.objects.getValue(id).copy(tapped = false)
        }
    }
     ...
     */
    /** "I cast Walking Ballista for X=3 and activate it": the permanent just cast, made the object the cast puts onto the
     *  battlefield rather than a second copy of it. The cast event is pointed at the object, which starts in hand. */
    private fun linkCastToObject(card: NameIndex.Entry, who: String, ctx: Ctx): String {
        val id = addObject(card, who, false, ctx, zone = "hand", allowDuplicate = true)
        val i = ctx.events.indexOfLast { it.verb == "cast" && it.player == who && it.obj == null && it.card?.name == card.display }
        if (i >= 0) ctx.events[i] = ctx.events[i].copy(obj = id)
        return id
    }

    /** "my commander" / "their commander" with no card named: a stand-in legendary creature that is a commander. */
    private fun commanderObject(who: String, ctx: Ctx): String {
        ctx.objects.values.lastOrNull { it.commander && it.controller == who }?.let { return it.id }
        var id = "commander"; var k = 2; while (ctx.objects.containsKey(id)) id = "commander_" + (k++)
        ctx.objects[id] = ObjectSpec(id, CardRef(name = "a commander"), controller = who, commander = true)
        ctx.notes += "${if (who == "me") "Your" else "Their"} commander stands in for whichever card it is, as a 2/2 legendary creature."
        return id
    }
    ...
    private fun applyStateWords(id: String, rest: String, ctx: Ctx) {
        ...
        // "Walking Ballista with 2 counters": the kind left unsaid on a creature is +1/+1, as it is elsewhere.
        Regex("""(?:with|has|having) (\d+|one|two|three|four|five|six|seven|eight|nine|ten|a|an) counters?(?: on it)?(?! \S)""").find(rest)?.let { r ->
            if (Regex("""(?:with|has|having) (?:(?:\d+|\w+) )?(?:[+-]\d+/[+-]\d+|[a-z]+) counters?""").find(rest)?.let { k -> k.groupValues[0].contains("+1/+1") || k.groupValues[0].contains("-1/-1") } == true) return@let
            val n = if (r.groupValues[1] in setOf("a", "an")) 1 else number(r.groupValues[1]) ?: return@let
            if (!isCreatureName(spec.card.name)) return@let
            // "Ballista with 3 counters … remove two counters from Ballista": the second mention restates the same
            // creature, so the larger count stands rather than the two adding up.
            counters["+1/+1"] = maxOf(counters["+1/+1"] ?: 0, n)
            ctx.notes += "\"$n counter${if (n == 1) "" else "s"}\" on ${spec.card.name} is read as +1/+1 counters; say the kind if you meant another."
        }
    ...
    private fun number(w: String): Int? = w.toIntOrNull() ?: numberWords[w] ?: when (w) { "a", "an", "one", "another" -> 1; else -> null }
